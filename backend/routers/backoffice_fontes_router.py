...
def detalhar_fonte(fonte_id: str, usuario: UsuarioAutenticado = Depends(_admin_master)):
    ...
    eventos = _dados(
        supabase.table("cti_fontes_eventos")
        .select("*")
        .eq("fonte_id", fonte_id)
        .order("created_at", desc=True)
        .execute()
    )
    return {"fonte": fonte[0], "eventos": eventos}


@router.post("/upload")
async def receber_fonte(
    ...
    if len(conteudo) > MAX_BYTES:
        raise HTTPException(status_code=413, detail="Arquivo excede o limite de 50 MB desta execução.")
    ...
    try:
        supabase.storage.from_(BUCKET).upload(
            storage_path,
            conteudo,
            file_options={"content-type": arquivo.content_type or "application/octet-stream", "upsert": "false"},
        )
    ...
    payload = {
        ...
        "tipo_detectado": tipo,
        "classificacao_negocio": "NAO_CLASSIFICADA",
        "status_governanca": "RECEBIDO",
        "interpretacao_resumo": {
            "etapa": "METADADOS_IDENTIFICADOS",
            "tipo_arquivo": tipo,
    ...
    try:
        ...
        if not criado:
            raise RuntimeError("Registro da fonte não retornado.")
        _registrar_evento(
            fonte_id,
            "FONTE_RECEBIDA",
            usuario.id,
...
def atualizar_governanca(
    fonte_id: str,
    request: AtualizarGovernancaRequest,
    usuario: UsuarioAutenticado = Depends(_admin_master),
):
    ...
    _registrar_evento(
        ...
        anterior=anterior,
        novo=novo,
        detalhes={"observacao": request.observacao, "classificacao_negocio": request.classificacao_negocio},
    )
