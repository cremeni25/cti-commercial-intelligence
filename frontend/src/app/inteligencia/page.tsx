...
type Copy = {
  eyebrow: string; title: string; intro: string; separation: string; context: string; segment: string; period: string; comparison: string; start: string; end: string; all: string;
  apply: string; clear: string; exportCsv: string; exportXlsx: string; loading: string; loadError: string; brazil: string; vienna: string; others: string; noComparison: string; previousPeriod: string; previousYear: string; periods: readonly string[];
  executive: string; marketVolume: string; trend: string; carrierPresence: string; classificationCoverage: string; growth: string; decline: string; stable: string; versus: string; records: string; classified: string;
  segmentsTitle: string; segmentsHelp: string; market: string; carrier: string; directCompetition: string; noParticipation: string; noContact: string; competitive: string; competitiveHelp: string;
  causes: string; causesHelp: string; originalReasons: string; observations: string; opportunities: string; opportunitiesHelp: string; client: string; score: string; occurrences: string; mainCause: string; competitor: string; implementer: string; months: string;
  evolution: string; implementers: string; clients: string; methodological: string; source: string; nature: string; updated: string; backend: string; recordsLabel: string; customersHistorical: string; currencyRecorded: string; avgTicket: string; unknown: string; notAvailable: string; evidence: string;
}
...
const ui: Record<"pt-BR" | "en" | "es", Copy> = {
  "pt-BR": { eyebrow: "CTI · inteligência ANFIR", title: "Inteligência de Mercado", intro: "Leitura executiva do mercado realizado ANFIR, separada do Funil e do CRM operacional.", separation: "ANFIR = mercado realizado. CRM = operação comercial. Funil = oportunidades. A análise correlaciona as camadas sem fundir registros.", context: "Contexto", segment: "Segmento", period: "Período", comparison: "Comparação", start: "Início", end: "Fim", all: "Todos", apply: "Aplicar filtros", clear: "Limpar", exportCsv: "Exportar CSV", exportXlsx: "Exportar XLSX", loading: "Carregando inteligência de mercado...", loadError: "Falha ao carregar", brazil: "Brasil", vienna: "VIENA SP", others: "Outros Dealers", noComparison: "Sem comparação", previousPeriod: "Período anterior", previousYear: "Ano anterior", periods: ["Hoje", "Últimos 7 dias", "Últimos 30 dias", "Últimos 90 dias", "Mês atual", "Trimestre atual", "Ano atual", "Personalizado"], executive: "Leitura executiva", marketVolume: "Mercado no período", trend: "Tendência de mercado", carrierPresence: "Presença Carrier observada", classificationCoverage: "Cobertura da classificação", growth: "crescimento", decline: "retração", stable: "estável", versus: "vs. período comparado", records: "registros", classified: "classificados", segmentsTitle: "Os três segmentos", segmentsHelp: "Comparação simultânea de Trailer, Diesel Truck e Direct Drive no mesmo contexto e período.", market: "Mercado", carrier: "Carrier observada", directCompetition: "Concorrência direta", noParticipation: "Não participamos", noContact: "Sem contato", competitive: "Panorama competitivo observado", competitiveHelp: "Distribuição dos fatos ANFIR classificados. Não representa market share contábil reconciliado.", causes: "Por que estamos perdendo espaço", causesHelp: "Causas estratégicas derivadas dos motivos e observações originais, sem substituir o texto fonte.", originalReasons: "Motivos originais Carrier", observations: "Sinais das observações", opportunities: "Onde agir primeiro", opportunitiesHelp: "Prioridades de recuperação calculadas por volume, categoria competitiva e causa estratégica.", client: "Cliente", score: "Prioridade", occurrences: "Ocorrências", mainCause: "Causa principal", competitor: "Cenário", implementer: "Implementadora", months: "meses", evolution: "Evolução mensal do mercado", implementers: "Implementadoras com maior volume", clients: "Clientes com maior volume", methodological: "Critérios metodológicos", source: "Origem", nature: "Natureza", updated: "Atualização", backend: "Backend respondeu", recordsLabel: "Registros realizados", customersHistorical: "Clientes observados", currencyRecorded: "Valor registrado", avgTicket: "Ticket médio histórico", unknown: "Não classificado", notAvailable: "Sem dados", evidence: "Evidência ANFIR realizada" },
  en: { eyebrow: "CTI · ANFIR intelligence", title: "Market Intelligence", intro: "Executive reading of realized ANFIR market facts, separated from Funnel and operational CRM.", separation: "ANFIR = realized market. CRM = commercial operations. Funnel = opportunities. Analysis correlates layers without merging records.", context: "Context", segment: "Segment", period: "Period", comparison: "Comparison", start: "Start", end: "End", all: "All", apply: "Apply filters", clear: "Clear", exportCsv: "Export CSV", exportXlsx: "Export XLSX", loading: "Loading market intelligence...", loadError: "Failed to load", brazil: "Brazil", vienna: "VIENA SP", others: "Other Dealers", noComparison: "No comparison", previousPeriod: "Previous period", previousYear: "Previous year", periods: ["Today", "Last 7 days", "Last 30 days", "Last 90 days", "Current month", "Current quarter", "Current year", "Custom"], executive: "Executive reading", marketVolume: "Market in period", trend: "Market trend", carrierPresence: "Observed Carrier presence", classificationCoverage: "Classification coverage", growth: "growth", decline: "decline", stable: "stable", versus: "vs. comparison period", records: "records", classified: "classified", segmentsTitle: "Three segments", segmentsHelp: "Side-by-side view of Trailer, Diesel Truck and Direct Drive in the same context and period.", market: "Market", carrier: "Observed Carrier", directCompetition: "Direct competition", noParticipation: "No participation", noContact: "No contact", competitive: "Observed competitive landscape", competitiveHelp: "Distribution of classified ANFIR facts. This is not reconciled accounting market share.", causes: "Why we are losing space", causesHelp: "Strategic causes derived from original reasons and notes without replacing source text.", originalReasons: "Original Carrier reasons", observations: "Signals from notes", opportunities: "Where to act first", opportunitiesHelp: "Recovery priorities calculated from volume, competitive category and strategic cause.", client: "Account", score: "Priority", occurrences: "Occurrences", mainCause: "Main cause", competitor: "Scenario", implementer: "Body Builder", months: "months", evolution: "Monthly market evolution", implementers: "Body Builders with highest volume", clients: "Accounts with highest volume", methodological: "Methodological criteria", source: "Source", nature: "Nature", updated: "Updated", backend: "Backend returned", recordsLabel: "Realized records", customersHistorical: "Observed accounts", currencyRecorded: "Recorded value", avgTicket: "Historical average ticket", unknown: "Unclassified", notAvailable: "No data", evidence: "Realized ANFIR evidence" },
  es: { eyebrow: "CTI · inteligencia ANFIR", title: "Inteligencia de Mercado", intro: "Lectura ejecutiva del mercado realizado ANFIR, separada del Embudo y del CRM operativo.", separation: "ANFIR = mercado realizado. CRM = operación comercial. Embudo = oportunidades. El análisis correlaciona las capas sin fusionar registros.", context: "Contexto", segment: "Segmento", period: "Período", comparison: "Comparación", start: "Inicio", end: "Fin", all: "Todos", apply: "Aplicar filtros", clear: "Limpiar", exportCsv: "Exportar CSV", exportXlsx: "Exportar XLSX", loading: "Cargando inteligencia de mercado...", loadError: "Error al cargar", brazil: "Brasil", vienna: "VIENA SP", others: "Otros Dealers", noComparison: "Sin comparación", previousPeriod: "Período anterior", previousYear: "Año anterior", periods: ["Hoy", "Últimos 7 días", "Últimos 30 días", "Últimos 90 días", "Mes actual", "Trimestre actual", "Año actual", "Personalizado"], executive: "Lectura ejecutiva", marketVolume: "Mercado en el período", trend: "Tendencia de mercado", carrierPresence: "Presencia Carrier observada", classificationCoverage: "Cobertura de clasificación", growth: "crecimiento", decline: "retracción", stable: "estable", versus: "vs. período comparado", records: "registros", classified: "clasificados", segmentsTitle: "Los tres segmentos", segmentsHelp: "Comparación simultánea de Trailer, Diesel Truck y Direct Drive en el mismo contexto y período.", market: "Mercado", carrier: "Carrier observada", directCompetition: "Competencia directa", noParticipation: "No participamos", noContact: "Sin contacto", competitive: "Panorama competitivo observado", competitiveHelp: "Distribución de hechos ANFIR clasificados. No representa market share contable reconciliado.", causes: "Por qué estamos perdiendo espacio", causesHelp: "Causas estratégicas derivadas de motivos y observaciones originales, sin sustituir el texto fuente.", originalReasons: "Motivos originales Carrier", observations: "Señales de observaciones", opportunities: "Dónde actuar primero", opportunitiesHelp: "Prioridades de recuperación calculadas por volumen, categoría competitiva y causa estratégica.", client: "Cliente", score: "Prioridad", occurrences: "Ocurrencias", mainCause: "Causa principal", competitor: "Escenario", implementer: "Carrocero", months: "meses", evolution: "Evolución mensual del mercado", implementers: "Carroceros con mayor volumen", clients: "Clientes con mayor volumen", methodological: "Criterios metodológicos", source: "Origen", nature: "Naturaleza", updated: "Actualización", backend: "Backend respondió", recordsLabel: "Registros realizados", customersHistorical: "Clientes observados", currencyRecorded: "Valor registrado", avgTicket: "Ticket medio histórico", unknown: "No clasificado", notAvailable: "Sin datos", evidence: "Evidencia ANFIR realizada" },
}
...
function qs(filters: Filters, extra: Record<string, string> = {}) {
  ...
  Object.entries({ ...filters, ...extra }).forEach(([key, value]) => { if (value) params.set(key, value) })
  return params.toString()
}
function pct(value: number) { return `${value > 0 ? "+" : ""}${Number(value || 0).toFixed(1)}%` }
function trendLabel(direction: string, copy: Pick<Copy, "growth" | "decline" | "stable">) { return direction === "alta" ? copy.growth : direction === "queda" ? copy.decline : copy.stable }
...
function MiniBar({ label, value, max }: { label: string; value: number; max: number }) {
  return <div><div className="flex items-center justify-between gap-3 text-sm"><span className="truncate">{label}</span><strong>{value}</strong></div><div className="mt-1 h-2 rounded-full bg-slate-100"><div className="h-2 rounded-full bg-blue-600" style={{ width: barWidth(value, max) }} /></div></div>
}

export default function InteligenciaPage() {
  const { locale, formatCurrency, formatDate, formatNumber } = useI18n()
  const lang: "pt-BR" | "en" | "es" = locale === "pt-BR" ? "pt-BR" : locale === "en" ? "en" : "es"
  const c = ui[lang]
  ...
  useEffect(() => {
    ...
    const request = { cache: "no-store" as RequestCache, signal: controller.signal }
    const intelligence = (segmento: string) => fetch(`${API_URL}/analytics/intelligence?${qs({ ...filters, segmento })}`, request)
    Promise.all([intelligence(filters.segmento), fetch(`${API_URL}/analytics/intelligence/filter-options?${query}`, request), intelligence("TR"), intelligence("DT"), intelligence("DD")])
      .then(async ([main, opts, tr, dt, dd]) => {
        if (![main, opts, tr, dt, dd].every(response => response.ok)) throw new Error(`${c.backend} ${[main, opts, tr, dt, dd].map(response => response.status).join("/")}`)
        const [mainData, optionData, trData, dtData, ddData] = await Promise.all([main.json(), opts.json(), tr.json(), dt.json(), dd.json()])
        setData(mainData); setOptions(optionData.opcoes || {}); setSegments({ TR: trData, DT: dtData, DD: ddData }); setError(null)
      })
      .catch((err: unknown) => { if (err instanceof DOMException && err.name === "AbortError") return; setError(err instanceof Error ? err.message : c.loadError) })
      .finally(() => { if (!controller.signal.aborted) setLoading(false) })
    return () => controller.abort()
  }, [c.backend, c.loadError, filters, query])

  function change(key: keyof Filters, value: string) { const next = { ...edit, [key]: value }; if (key === "linha") { next.familia = ""; next.produto = "" }; if (key === "familia") next.produto = ""; setEdit(next) }
  ...
  return <main className="min-h-screen bg-slate-50 p-4 md:p-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    {loading && <div className="rounded-2xl border bg-white p-10 text-center">{c.loading}</div>}{error && <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-red-800">{error}</div>}{!loading && data?.empty_state && <div className="rounded-2xl border bg-white p-10 text-center">{data.empty_state}</div>}
    {!loading && data && !data.empty_state && <>
      <section><h2 className="mb-3 text-xl font-semibold">{c.executive}</h2><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><MetricCard title={c.marketVolume} value={`${formatNumber(market?.mercado.volume ?? data.kpis.volume)} ${c.records}`} detail={comparison ? `${pct(comparison.percentual)} ${c.versus}` : undefined} /><MetricCard title={c.trend} value={comparison ? trendLabel(comparison.direcao, c) : c.notAvailable} detail={comparison ? `${formatNumber(comparison.atual)} × ${formatNumber(comparison.anterior)} ${c.records}` : undefined} /><MetricCard title={c.carrierPresence} value={`${(market?.competitividade.carrier_observado.participacao_observada_percentual ?? 0).toFixed(1)}%`} detail={`${formatNumber(market?.competitividade.carrier_observado.quantidade ?? 0)} ${c.records}`} /><MetricCard title={c.classificationCoverage} value={`${(market?.competitividade.cobertura_classificacao_percentual ?? 0).toFixed(1)}%`} detail={`${formatNumber(market?.competitividade.registros_classificados ?? 0)} ${c.classified}`} /></div></section>
      <section className="rounded-2xl border bg-white p-5 shadow-sm"><h2 className="text-xl font-semibold">{c.segmentsTitle}</h2><p className="mt-1 text-sm text-slate-500">{c.segmentsHelp}</p><div className="mt-5 grid gap-4 lg:grid-cols-3">{(["TR", "DT", "DD"] as SegmentCode[]).map(code => { const item = segments[code]?.inteligencia_mercado; const comp = item?.mercado.comparacao; return <div key={code} className="rounded-xl border p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-blue-700">{code}</p><h3 className="text-lg font-semibold">{segmentNames[code]}</h3></div><span className={`rounded-full px-2 py-1 text-xs font-medium ${comp?.direcao === "alta" ? "bg-emerald-50 text-emerald-700" : comp?.direcao === "queda" ? "bg-red-50 text-red-700" : "bg-slate-100 text-slate-600"}`}>{comp ? `${pct(comp.percentual)} · ${trendLabel(comp.direcao, c)}` : c.notAvailable}</span></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><p className="text-slate-500">{c.market}</p><strong>{formatNumber(item?.mercado.volume ?? 0)}</strong></div><div><p className="text-slate-500">{c.carrier}</p><strong>{(item?.competitividade.carrier_observado.participacao_observada_percentual ?? 0).toFixed(1)}%</strong></div><div><p className="text-slate-500">{c.directCompetition}</p><strong>{formatNumber(item?.cobertura_comercial.concorrencia_direta_observada ?? 0)}</strong></div><div><p className="text-slate-500">{c.noParticipation}</p><strong>{formatNumber(item?.cobertura_comercial.nao_participamos_proposta ?? 0)}</strong></div></div></div> })}</div></section>
      <section className="grid gap-6 lg:grid-cols-2"><div className="rounded-2xl border bg-white p-5"><h2 className="font-semibold">{c.competitive}</h2><p className="mt-1 text-sm text-slate-500">{c.competitiveHelp}</p><div className="mt-5 space-y-4">{(market?.competitividade.distribuicao || []).filter(item => item.quantidade > 0).map(item => <MiniBar key={item.categoria} label={categoryNames[lang][item.categoria] || item.categoria} value={item.quantidade} max={competitiveMax} />)}</div></div><div className="rounded-2xl border bg-white p-5"><h2 className="font-semibold">{c.causes}</h2><p className="mt-1 text-sm text-slate-500">{c.causesHelp}</p><div className="mt-5 space-y-4">{(market?.causas_estrategicas || []).slice(0, 9).map(item => <MiniBar key={item.causa} label={causeNames[lang][item.causa] || item.causa} value={item.quantidade} max={causesMax} />)}</div></div></section>
