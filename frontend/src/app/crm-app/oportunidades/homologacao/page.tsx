...
export default function HomologacaoPage() {
  ...
  return <main className="min-h-[100dvh] bg-[#020817] px-4 py-5 pb-24 text-white sm:px-6"><div className="mx-auto max-w-5xl">
    ...
    <section className="rounded-3xl border border-amber-800/60 bg-amber-950/15 p-5 sm:p-6">
      ...
      {previa && <>
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-5">{Object.entries(previa.resumo).map(([chave, valor]) => <div key={chave} className="min-w-0 rounded-2xl border border-[#24466f] bg-[#07162b] p-3"><strong className="block text-xl text-cyan-300">{valor}</strong><span className="block break-words text-[11px] leading-4 text-slate-400 sm:text-xs">{chave.replaceAll("_", " ")}</span></div>)}</div>
