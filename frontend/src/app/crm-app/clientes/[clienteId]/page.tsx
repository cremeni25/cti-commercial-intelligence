    ...
    <div className="mx-auto max-w-5xl">
      ...
      {carregando ? <div className="grid min-h-72 place-items-center"><Loader2 className="animate-spin text-cyan-300"/></div> : cliente && <>
        <section className="mb-4 rounded-3xl border border-[#16325c] bg-gradient-to-br from-[#0a2242] to-[#07162b] p-5">
          <div className="flex items-start gap-3"><span className="rounded-2xl bg-cyan-950/50 p-3 text-cyan-300"><Building2/></span><div><h2 className="text-xl font-bold">{cliente.nome}</h2><p className="mt-1 text-sm text-slate-400">{cliente.codigo}</p>{cliente.cidade && <p className="mt-1 flex items-center gap-1 text-sm text-slate-300"><MapPin size={15}/>{cliente.cidade}{cliente.uf ? `/${cliente.uf}` : ""}</p>}</div></div>
