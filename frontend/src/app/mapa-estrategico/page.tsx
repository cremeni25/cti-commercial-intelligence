          ...
          <nav className="flex flex-wrap gap-2" aria-label="Visões da Inteligência Comercial">
            {visoes.map((item) => (
              <button
                ...
                className={`rounded-xl border px-4 py-2.5 text-sm font-semibold transition ${visao === item.id ? "border-cyan-400 bg-cyan-400 text-slate-950" : "border-[#214363] bg-[#071226] text-slate-300 hover:border-cyan-500/60 hover:text-white"}`}
              >
                {item.label}
              </button>
            ))}
          ...
          {!loading && dados && (
            <details className="group rounded-2xl border border-slate-700/60 bg-[#061126] px-5 py-4">
              <summary className="cursor-pointer list-none text-sm font-semibold text-slate-300">
                Auditoria e origem dos dados
                <span className="ml-2 text-xs text-slate-500">ANFIR · Histórico/Funil · CRM</span>
              </summary>
...
function VisaoExecutiva({
  ...
  return <>
    ...
    <section className="grid gap-4 xl:grid-cols-[.9fr_1.1fr]">
      <div className="rounded-3xl border border-[#17304d] bg-[#061126] p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[.16em] text-slate-500">Base comercial 2026</p>
            <h2 className="mt-1 text-xl font-bold">Tamanho e composição</h2>
          </div>
      ...
      <div className="rounded-3xl border border-violet-500/30 bg-[#081126] p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[.16em] text-violet-300">IA Comercial CTI</p>
            <h2 className="mt-1 text-xl font-bold">O que os dados estão mostrando</h2>
          </div>
          <div className="flex gap-2">
            ...
            <button type="button" onClick={aprofundarNaIa} className="rounded-xl bg-violet-400 px-3 py-2 text-xs font-bold text-slate-950">Aprofundar na IA</button>
          </div>
        </div>
    ...
    <section className="grid gap-4 xl:grid-cols-2">
      <button type="button" onClick={() => irPara("crm")} className="rounded-3xl border border-emerald-500/20 bg-[#061126] p-5 text-left transition hover:border-emerald-400/60">
        <div className="flex items-center justify-between gap-3"><h2 className="font-bold">CRM atual</h2><strong className="text-emerald-300">{dados.evidencias.crm_ativos} ativas</strong></div>
        <div className="mt-4 space-y-2">{dados.evidencias.crm_status.length ? dados.evidencias.crm_status.map((item) => <BarraStatus key={item.nome} nome={item.nome} valor={item.quantidade} total={Math.max(1, dados.evidencias.crm_registros)} />) : <p className="text-sm text-slate-500">Sem negociações ativas nesta seleção.</p>}</div>
      </button>
      <button type="button" onClick={() => irPara("historico")} className="rounded-3xl border border-amber-500/20 bg-[#061126] p-5 text-left transition hover:border-amber-400/60">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3"><MiniKpi rotulo="Eventos" valor={dados.evidencias.historico_registros_2026} /><MiniKpi rotulo="Perdas com motivo" valor={dados.evidencias.motivos_perda_historico.reduce((s, i) => s + i.quantidade, 0)} /></div>
      </button>
    </section>
  </>
}
...
function VisaoCrm({ dados, ticketPipeline }: { dados: MapaEquipeVisao; ticketPipeline: number }) {
  return <section className="space-y-5 rounded-3xl border border-emerald-500/20 bg-[#061126] p-5">
    <div><p className="text-xs uppercase tracking-[.16em] text-emerald-300">CRM atual</p><h2 className="mt-1 text-2xl font-bold">O que está em andamento em {dados.selecao.nome}</h2></div>
    <div className="grid gap-3 sm:grid-cols-3"><MiniKpi rotulo="Negociações ativas" valor={dados.evidencias.crm_ativos} /><MiniKpi rotulo="Pipeline ativo" valor={formatarMoeda(dados.evidencias.crm_valor_ativo)} /><MiniKpi rotulo="Ticket médio" valor={formatarMoeda(ticketPipeline)} /></div>
    <div className="space-y-3">{dados.evidencias.crm_status.length ? dados.evidencias.crm_status.map((item) => <BarraStatus key={item.nome} nome={item.nome} valor={item.quantidade} total={Math.max(1, dados.evidencias.crm_registros)} />) : <p className="text-sm text-slate-500">Sem registros operacionais nesta seleção.</p>}</div>
  </section>
}

function VisaoHistorico({ dados, perdasHistorico }: { dados: MapaEquipeVisao; perdasHistorico: number }) {
  return <section className="space-y-5 rounded-3xl border border-amber-500/20 bg-[#061126] p-5">
    <div><p className="text-xs uppercase tracking-[.16em] text-amber-300">Histórico / Funil 2026</p><h2 className="mt-1 text-2xl font-bold">O que ficou registrado antes para {dados.selecao.nome}</h2></div>
    <div className="grid gap-3 sm:grid-cols-3"><MiniKpi rotulo="Eventos" valor={dados.evidencias.historico_registros_2026} /><MiniKpi rotulo="Unidades" valor={dados.evidencias.historico_unidades_2026.toLocaleString("pt-BR")} /><MiniKpi rotulo="Perdas com motivo" valor={perdasHistorico} /></div>
    <div className="space-y-3">{dados.evidencias.motivos_perda_historico.length ? dados.evidencias.motivos_perda_historico.map((item) => <BarraStatus key={item.nome} nome={item.nome} valor={item.quantidade} total={Math.max(1, perdasHistorico)} />) : <p className="text-sm text-slate-500">Não há motivo de perda estruturado suficiente nesta seleção.</p>}</div>
  </section>
}
...
function BarraMercado({ total, fora, real }: { total: number; fora: number; real: number }) {
  ...
  return <div>
    <div className="mb-2 flex flex-wrap justify-between gap-2 text-xs"><span className="text-slate-400">Mercado territorial observado: <strong className="text-white">{total}</strong></span><span><span className="text-amber-300">Fora da disputa {fora} · {foraPct.toFixed(1)}%</span><span className="ml-3 text-emerald-300">Mercado Real {real} · {realPct.toFixed(1)}%</span></span></div>
    <div className="flex h-4 overflow-hidden rounded-full bg-slate-900"><div className="bg-amber-500" style={{ width: `${foraPct}%` }} /><div className="bg-emerald-500" style={{ width: `${realPct}%` }} /></div>
  </div>
}
