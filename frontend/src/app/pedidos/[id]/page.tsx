...
export default function PedidoDetalhesPage() {
  ...
  return <main className="flex min-h-screen bg-[#020817] text-white"><Sidebar /><section className="min-w-0 flex-1"><Topbar /><div className="space-y-6 p-4 sm:p-6 lg:p-8">
    <header className="rounded-3xl border border-[#13203f] bg-[#091a33] p-6"><Link href="/pedidos" className="text-sm font-semibold text-cyan-300">← Voltar para pedidos</Link><div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-cyan-400">Dossiê comercial</p><h1 className="mt-2 text-3xl font-bold">{String(dados?.pedido.numero || "Pedido")}</h1><p className="mt-2 text-sm text-slate-400">Proposta aceita, aceite registrado e documentos CARRIER.</p></div>{dados && <span className="w-fit rounded-full border border-cyan-800 px-4 py-2 text-sm text-cyan-200">{String(dados.pedido.status_envio_carrier || "NAO_ENVIADO")}</span>}</div></header>
    {loading && <Aviso>Carregando dossiê...</Aviso>}{erro && <div className="rounded-xl border border-red-900 bg-red-950/30 p-4 text-red-200">{erro}</div>}{mensagem && <div className="rounded-xl border border-emerald-900 bg-emerald-950/30 p-4 text-emerald-200">{mensagem}</div>}
    {dados && <>
