...
export default function Page() {
  ...
  useEffect(() => {
    ...
    Promise.all([getMapaEquipeVisao(responsavelId || null), getMapaInsights(responsavelId || null)])
      ...
      .catch((e) => {
        if (ativo) setErro(e instanceof Error ? e.message : "Não foi possível carregar a visão comercial.")
      })
      .finally(() => {
    ...
    void (async () => {
      try {
        ...
        const resposta = await fetch(
          `/api/cti/analytics/anfir-workbook-2026${qs.toString() ? `?${qs.toString()}` : ""}`,
          {
            cache: "no-store",
        ...
        setMercadoMacro({
          ...
          segmentos: Array.isArray(payload.inteligencia_viena?.segmentos)
            ? payload.inteligencia_viena!.segmentos!
            : [],
        })
      } catch {
  ...
  const familiaTotal = useMemo(
    () =>
      dados
        ? dados.mercado.familias.trailer +
          dados.mercado.familias.diesel_truck +
          dados.mercado.familias.direct_drive
        : 0,
    [dados],
  )
    ...
    <main className="flex min-h-screen bg-[#020817] text-white">
      ...
      <section className="min-w-0 flex-1">
        ...
        <div className="space-y-4 p-4 sm:p-6 lg:p-8">
          <header className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm text-slate-400">
                Mercado, evolução competitiva, acompanhamento e resultado comercial de 2026.
              </p>
            </div>
          ...
          {loading && (
            <div className="rounded-2xl border border-[#17304d] bg-[#071226] p-6 text-slate-400">
              Carregando informações comerciais...
            </div>
          )}
...
function VisaoComercial({
  ...
  setFoco: (f: FocoInteligencia) => void
  responsavelId?: string
}) {
  const consolidado = insights.escopo.consolidado
  const ticketPipeline = dados.evidencias.crm_ativos
    ? dados.evidencias.crm_valor_ativo / dados.evidencias.crm_ativos
    : 0
  const hrefAnfir = mapaDetalhe({
    camada: "anfir",
    responsavelId,
    titulo: "Mercado ANFIR 2026",
    subtitulo: "Unidades que formam exatamente este total",
  })
  const hrefCrm = mapaDetalhe({
    camada: "crm",
    responsavelId,
    titulo: "Negociações em andamento",
    subtitulo: "Negócios ativos que formam exatamente este total",
  })
  const valorMercado =
    consolidado && !responsavelId && mercadoMacro
      ? mercadoMacro.real
      : dados.mercado.mercado_real_selecao_2026
  const hrefMercado =
    consolidado && !responsavelId
      ? detalheGeral({
          camada: "anfir",
          mercado: "DISPUTAVEL_VIENA",
          titulo: "Mercado Real Viena 2026",
          subtitulo: "Registros após retirada do mercado fora do escopo comercial",
        })
      : hrefAnfir
  ...
  return (
    <>
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi
          href={hrefMercado}
          titulo={consolidado && !responsavelId ? "Mercado Real Viena" : "Meu mercado 2026"}
          valor={valorMercado}
          apoio="ANFIR 2026 · abrir unidades"
          destaque="emerald"
        />
        <Kpi
          href={hrefAnfir}
          titulo={consolidado ? "Mercado identificado" : "Registros no meu mercado"}
          valor={dados.mercado.mercado_real_selecao_2026}
          apoio={`${dados.selecao.nome} · abrir evidências`}
          destaque="cyan"
        />
        <Kpi
          href={hrefCrm}
          titulo="Negociações em andamento"
          valor={dados.evidencias.crm_ativos}
          apoio="CRM atual · abrir negócios"
          destaque="emerald"
        />
        <Kpi
          href={hrefCrm}
          titulo="Pipeline atual"
          valor={formatarMoeda(dados.evidencias.crm_valor_ativo)}
          apoio={
            dados.evidencias.crm_ativos
              ? `ticket médio ${formatarMoeda(ticketPipeline)}`
              : "sem negócios ativos"
          }
        />
      </section>
          ...
          <button
            ...
            className={`rounded-2xl border p-4 text-left transition ${
              foco === item.id
                ...
                : "border-[#17304d] bg-[#071226] hover:border-cyan-500/40"
            }`}
          >
      ...
      {foco === "linhas" && <VisaoLinhas insights={insights} responsavelId={responsavelId} />}
      ...
      <details className="rounded-2xl border border-slate-700/50 bg-[#061126] px-5 py-4">
        ...
        <div className="mt-4 grid gap-3 border-t border-slate-700/50 pt-4 sm:grid-cols-3">
          ...
          <MiniKpi
            rotulo="Unidades históricas 2026"
            valor={dados.evidencias.historico_unidades_2026.toLocaleString("pt-BR")}
          />
    ...
    <section className="space-y-4">
      {consolidado && !responsavelId && (
        <div className="rounded-3xl border border-cyan-500/20 bg-[#061126] p-5">
          <p className="text-xs font-semibold uppercase tracking-[.16em] text-cyan-300">
            Base executiva auditável · ANFIR 2026
          </p>
          <h2 className="mt-1 text-xl font-bold">Mercado total → retiradas → mercado real Viena</h2>
          <p className="mt-2 text-sm text-slate-400">
            Número e percentual sobre a mesma base. O mercado total e o mercado real podem ser abertos até os registros que os formam.
          </p>
          {mercadoMacro ? (
            <div className="mt-5">
            ...
            <p className="mt-4 text-sm text-slate-500">Aguardando composição auditável do mercado.</p>
          )}
        </div>
      )}
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-3xl border border-[#17304d] bg-[#061126] p-5">
          <p className="text-xs font-semibold uppercase tracking-[.16em] text-cyan-300">Composição por linha</p>
          <h2 className="mt-1 text-xl font-bold">Onde está o volume da seleção</h2>
          <div className="mt-5 space-y-3">
            <BarraComercial
              href={mapaDetalhe({
                ...
                responsavelId,
                familia: "trailer",
                titulo: "Trailer · ANFIR 2026",
                subtitulo: "Unidades que formam o total de Trailer",
              })}
              nome="Trailer"
              valor={dados.mercado.familias.trailer}
              total={Math.max(1, familiaTotal)}
            />
            <BarraComercial
              href={mapaDetalhe({
                camada: "anfir",
                responsavelId,
                familia: "diesel-truck",
                titulo: "Diesel Truck · ANFIR 2026",
                subtitulo: "Unidades que formam o total de Diesel Truck",
              })}
              nome="Diesel Truck"
              valor={dados.mercado.familias.diesel_truck}
              total={Math.max(1, familiaTotal)}
            />
            <BarraComercial
              href={mapaDetalhe({
                camada: "anfir",
                responsavelId,
                familia: "direct-drive",
                titulo: "Direct Drive · ANFIR 2026",
                subtitulo: "Unidades que formam o total de Direct Drive",
              })}
              nome="Direct Drive"
              valor={dados.mercado.familias.direct_drive}
              total={Math.max(1, familiaTotal)}
            />
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-violet-500/20 bg-[#061126] p-5">
          ...
          <h2 className="mt-1 text-xl font-bold">Mercado → Histórico/Funil → CRM → evidência ANFIR</h2>
          <p className="mt-2 text-sm text-slate-400">
            As fontes permanecem separadas; a inteligência mostra onde houve acompanhamento e onde o mercado apareceu sem evidência comercial anterior.
          </p>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            <MiniKpi rotulo="Clientes no mercado" valor={dados.ciclo.clientes_mercado_real} />
            <MiniKpi rotulo="No Histórico/Funil" valor={dados.ciclo.clientes_historico_2026} />
            <MiniKpi rotulo="No CRM" valor={dados.ciclo.clientes_crm} />
            <MiniKpi rotulo="CRM + ANFIR" valor={dados.ciclo.crm_com_evidencia_anfir} />
            <MiniKpi rotulo="Mercado sem CRM" valor={semCrm} destaque="amber" />
          </div>
          <div className="mt-4 flex flex-wrap gap-2 text-xs">
            <Link
              className="rounded-lg border border-cyan-700 px-3 py-2 text-cyan-200"
              href={mapaDetalhe({
                camada: "anfir",
                responsavelId,
                titulo: "ANFIR 2026",
                subtitulo: "Registros do mercado observado",
              })}
            >
              Abrir ANFIR
            </Link>
            <Link
              className="rounded-lg border border-violet-700 px-3 py-2 text-violet-200"
              href={mapaDetalhe({
                camada: "historico",
                responsavelId,
                titulo: "Histórico/Funil 2026",
                subtitulo: "Registros históricos da seleção",
              })}
            >
              Abrir Histórico/Funil
            </Link>
            <Link
              className="rounded-lg border border-emerald-700 px-3 py-2 text-emerald-200"
              href={mapaDetalhe({
                camada: "crm",
                responsavelId,
                titulo: "CRM atual",
                subtitulo: "Negócios ativos da seleção",
              })}
            >
              Abrir CRM
            </Link>
  ...
  return (
    <section className="space-y-4">
      <div className="rounded-3xl border border-cyan-500/20 bg-[#061126] p-5">
        ...
        <p className="mt-2 text-sm text-slate-400">
          Responsabilidade comercial prevalece; DDD não redistribui mercado entre pessoas.
        </p>
      </div>
        ...
        {insights.regioes.map((item) => (
          <article key={item.id} className="rounded-3xl border border-[#17304d] bg-[#061126] p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-[.14em] text-slate-500">Responsável comercial</p>
                <h3 className="mt-1 text-lg font-bold">{item.nome}</h3>
              </div>
              <Link
                href={mapaDetalhe({
                  camada: "anfir",
                  responsavelId: item.id,
                  titulo: `Mercado 2026 · ${item.nome}`,
                  subtitulo: "Unidades atribuídas a este responsável",
                })}
                className="text-right"
              >
                <strong className="text-2xl text-cyan-300 underline decoration-cyan-500/40 underline-offset-4">
                  {item.mercado_2026}
                </strong>
                <p className="text-xs text-slate-500">abrir unidades</p>
              </Link>
            </div>

            <div className="mt-4">
              <GraficoLinha valores={item.mercado_mensal} meses={insights.meses} rotulo="Movimento mensal" />
            </div>

            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              <MiniKpi rotulo="Clientes" valor={item.clientes_mercado} />
              <MiniKpi rotulo="Negócios ativos" valor={item.crm_ativos} />
...
function VisaoLinhas({ insights, responsavelId }: { insights: MapaInsights; responsavelId?: string }) {
  return (
    <section className="space-y-4">
      <div className="rounded-3xl border border-cyan-500/20 bg-[#061126] p-5">
        <p className="text-xs font-semibold uppercase tracking-[.16em] text-cyan-300">Evolução por linha · ANFIR 2026</p>
        <h2 className="mt-1 text-xl font-bold">Mercado primeiro. Marca depois.</h2>
        <p className="mt-2 text-sm text-slate-400">
          TR/Trailer, DT/Diesel Truck e DD/Direct Drive fecham primeiro o mercado da linha. Carrier e demais marcas são abertas depois, sempre sobre o mesmo total.
        </p>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        {insights.linhas_2026.linhas.map((linha) => {
          const familia =
            linha.codigo === "diesel_truck"
              ? "diesel-truck"
              : linha.codigo === "direct_drive"
                ? "direct-drive"
                : "trailer"
          ...
          return (
            <article key={linha.codigo} className="rounded-3xl border border-[#17304d] bg-[#061126] p-5">
              <div className="flex items-end justify-between gap-3">
                <div>
                  <p className="text-xs uppercase tracking-[.14em] text-slate-500">Mercado da linha</p>
                  <h3 className="mt-1 text-lg font-bold">{linha.nome}</h3>
                </div>
                <Link
                  href={mapaDetalhe({
                    camada: "anfir",
                    responsavelId,
                    familia,
                    titulo: `${linha.nome} · ANFIR 2026`,
                    subtitulo: "Unidades individualizadas desta linha",
                  })}
                  className="text-right"
                >
                  <strong className="text-2xl text-cyan-300 underline decoration-cyan-500/40 underline-offset-4">
                  ...
                  <span className="block text-[10px] text-slate-500">abrir unidades</span>
                </Link>
              </div>

              <div className="mt-4">
                <GraficoLinha
                  valores={linha.mensal}
                  meses={insights.linhas_2026.meses}
                  rotulo={`Mercado mensal de ${linha.nome}`}
                />
              </div>
      ...
      {insights.linhas_2026.nao_classificado_2026 > 0 && (
        <p className="px-1 text-xs text-amber-300">
          Há {insights.linhas_2026.nao_classificado_2026} unidade(s) ainda sem linha classificada; nenhuma foi forçada para TR, DT ou DD.
        </p>
      )}
    </section>
  )
...
function VisaoPerdas({ insights, responsavelId }: { insights: MapaInsights; responsavelId?: string }) {
  ...
  return (
    <section className="space-y-4">
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[.16em] text-amber-300">
              Perdas comerciais · ANFIR 2026
            </p>
            <h2 className="mt-1 text-xl font-bold">Onde perdemos e por quê</h2>
          </div>
          <Link
            href={mapaDetalhe({
              camada: "anfir",
              responsavelId,
              somentePerdas: true,
              titulo: "Perdas identificadas · ANFIR 2026",
              subtitulo: "Somente os registros que compõem o total de perdas",
            })}
            className="text-right"
          >
            <strong className="text-3xl text-amber-300 underline decoration-amber-500/40 underline-offset-4">
              {insights.perdas.total_perdido}
            </strong>
            <p className="text-xs text-slate-500">abrir perdas</p>
          </Link>
        </div>
        <div className="mt-5">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-3xl border border-amber-500/20 bg-[#061126] p-5">
          <h3 className="text-lg font-bold">Por que perdemos</h3>
          <div className="mt-5 space-y-3">
            {insights.perdas.motivos.length ? (
              insights.perdas.motivos.map((i) => (
                <BarraComercial
                  key={i.nome}
                  nome={i.nome.replaceAll("_", " ")}
                  valor={i.quantidade}
                  total={totalMotivos}
                />
              ))
            ) : (
              <p className="text-sm text-slate-500">Sem motivo estruturado suficiente.</p>
            )}
          </div>
        </div>
        <div className="rounded-3xl border border-amber-500/20 bg-[#061126] p-5">
          <h3 className="text-lg font-bold">Onde perdemos</h3>
          <div className="mt-5 space-y-3">
            {insights.perdas.por_linha.length ? (
              insights.perdas.por_linha.map((i) => (
                <BarraComercial key={i.nome} nome={i.nome} valor={i.quantidade} total={totalLinhas} />
              ))
            ) : (
              <p className="text-sm text-slate-500">Sem linha classificada nas perdas.</p>
            )}
          </div>
        </div>
      </div>
...
function obterDirecionamento(item: unknown): Direcionamento | undefined {
  if (!item || typeof item !== "object") return undefined
  const valor = (item as { direcionamento?: Direcionamento }).direcionamento
  if (!valor || !Array.isArray(valor.alvos)) return undefined
  return valor
}
    ...
    <div>
      <div className="grid gap-3 sm:grid-cols-3">
        <Link
          href={detalheGeral({
            camada: "anfir",
            titulo: "Mercado total ANFIR 2026",
            subtitulo: "Todos os registros da base ANFIR no recorte autorizado",
          })}
        >
          <MiniKpi rotulo="1 · Mercado total ANFIR" valor={`${total} · 100%`} />
        </Link>
        ...
        <Link
          href={detalheGeral({
            ...
            mercado: "DISPUTAVEL_VIENA",
            titulo: "Mercado Real Viena 2026",
            subtitulo: "Registros após retirada das empresas fora do escopo comercial",
          })}
        >
          <MiniKpi rotulo="3 · Mercado Real Viena" valor={`${real} · ${realPct.toFixed(1)}%`} />
        </Link>
      </div>
      <div className="mt-4 flex h-5 overflow-hidden rounded-full bg-slate-900">
        <div className="bg-amber-500" style={{ width: `${foraPct}%` }} />
        <div className="bg-emerald-500" style={{ width: `${realPct}%` }} />
      </div>
      <div className="mt-2 flex justify-end gap-4 text-xs">
        <span className="text-amber-300">Retirado {foraPct.toFixed(1)}%</span>
...
function detalheGeral({
  ...
  if (camada !== "crm") {
    qs.set("contexto", "viena_sp")
    qs.set("periodo", "ANO_ATUAL")
  }
...
function Kpi({
  ...
  valor: string | number
  apoio?: string
  destaque?: "cyan" | "emerald"
  href?: string
}) {
  const cor =
    destaque === "emerald" ? "text-emerald-300" : destaque === "cyan" ? "text-cyan-300" : "text-white"
  const corpo = (
    <div className="rounded-2xl border border-[#17304d] bg-[#071226] p-4">
      <p className="text-[11px] font-semibold uppercase tracking-[.14em] text-slate-500">{titulo}</p>
      <div className={`mt-2 text-2xl font-bold ${cor}`}>{valor}</div>
...
function MiniKpi({
  ...
  return (
    <div className="rounded-xl border border-[#17304d] bg-[#09152a] p-3">
      <p className="text-[10px] uppercase tracking-[.12em] text-slate-500">{rotulo}</p>
      <strong className={`mt-1 block text-lg ${destaque === "amber" ? "text-amber-300" : "text-white"}`}>
        {valor}
      </strong>
    </div>
  )
...
function BarraComercial({
  ...
  const corpo = (
    <>
      ...
      <div className="h-3 overflow-hidden rounded-full bg-[#0b2040]">
        <div className="h-full rounded-full bg-cyan-400" style={{ width: `${Math.min(100, percentual)}%` }} />
      </div>
    </>
  )
  return href ? (
    <Link href={href} className="block rounded-lg p-1 hover:bg-cyan-500/5">
      {corpo}
    </Link>
  ) : (
    <div>{corpo}</div>
...
function LeituraAcao({
  ...
  return (
    <div className="mt-5 grid gap-3 border-t border-slate-700/50 pt-4 md:grid-cols-2">
      <div>
        <p
          className={`text-[11px] font-semibold uppercase tracking-[.14em] ${
            destaque === "amber" ? "text-amber-300" : "text-cyan-300"
          }`}
        >
          Leitura comercial
        </p>
      ...
      <div>
        <p className="text-[11px] font-semibold uppercase tracking-[.14em] text-emerald-300">O que fazer</p>
        <p className="mt-2 text-sm leading-6 text-slate-300">{acao}</p>
      </div>
    </div>
  )
}
...
function GraficoLinha({ valores, meses, rotulo }: { valores: number[]; meses: string[]; rotulo: string }) {
  ...
  const mx = 22
  const my = 18
  const passo = (w - mx * 2) / 11
  const pts = serie.map((valor, i) => ({
    x: mx + i * passo,
    y: h - my - (valor / max) * (h - my * 2),
    valor,
  }))
  const path = pts.map((p, i) => `${i ? "L" : "M"}${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(" ")

  return (
    <div className="rounded-2xl border border-[#17304d] bg-[#071226] p-3" aria-label={rotulo}>
      <svg viewBox={`0 0 ${w} ${h + 26}`} className="h-48 w-full">
        <line x1={mx} y1={h - my} x2={w - mx} y2={h - my} stroke="currentColor" className="text-slate-700" />
        <path
          d={path}
          fill="none"
          stroke="currentColor"
        ...
        {pts.map((p, i) => (
          <g key={i}>
            <circle cx={p.x} cy={p.y} r="4" fill="currentColor" className="text-cyan-300" />
            <text
              x={p.x}
              y={Math.max(12, p.y - 9)}
              textAnchor="middle"
