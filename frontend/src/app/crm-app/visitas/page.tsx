...
function lista(payload: unknown): Registro[] {
  ...
  if (payload && typeof payload === "object") {
    ...
    for (const chave of ["dados", "itens", "resultado", "atividades", "oportunidades"]) if (Array.isArray(objeto[chave])) return objeto[chave] as Registro[]
  }
  return []
}
...
export default function VisitasPage() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const oportunidadesNormalizadas = lista(oportunidadesPayload).map((item) => ({
        id: texto(item.id || item.oportunidade_id), clienteId: texto(item.cliente_id), cliente: texto(item.cliente_nome || item.cliente || item.empresa), titulo: texto(item.titulo || item.equipamento) || "Oportunidade",
      })).filter((item) => item.id)
  ...
  async function criarVisita(evento: FormEvent<HTMLFormElement>) {
    ...
    if (!usuario?.id || !texto(dados.get("cliente_id")) || !texto(dados.get("titulo")) || !texto(dados.get("objetivo"))) {
      setErro("Informe cliente, título, objetivo e confirme o usuário autenticado."); setSalvando(false); return
    }
    try {
      const resposta = await fetch("/api/crm-proxy/crm/atividades", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        cliente_id: texto(dados.get("cliente_id")), oportunidade_id: texto(dados.get("oportunidade_id")) || null,
        usuario_id: String(usuario.id), tipo: "VISITA", titulo: texto(dados.get("titulo")), descricao: descricao(texto(dados.get("objetivo"))),
        data: texto(dados.get("data")), horario: texto(dados.get("horario")), status: "PENDENTE",
      }) })
      ...
      evento.currentTarget.reset(); setClienteId(""); setNovaAberta(false); setSucesso("Visita agendada e sincronizada com o CTI."); await carregar()
  ...
  async function iniciar(visita: Visita) {
    setErro(""); setSucesso("")
    const resposta = await fetch(`/api/crm-proxy/crm/atividades/${encodeURIComponent(visita.id)}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "EM_ANDAMENTO" }) })
    const detalhe = await resposta.json().catch(() => ({})) as Registro
    if (!resposta.ok) return setErro(texto(detalhe.detail) || "Não foi possível iniciar a visita.")
    setSucesso("Visita iniciada. Registre o resultado ao final."); await carregar()
  }
  ...
  async function concluir(evento: FormEvent<HTMLFormElement>) {
    ...
    const dados = new FormData(evento.currentTarget)
    const resultado = texto(dados.get("resultado")); const desfecho = texto(dados.get("desfecho")); const proxima = texto(dados.get("proxima_acao")); const proximaData = texto(dados.get("proxima_data"))
    ...
    try {
      const resposta = await fetch("/api/crm-visitas/concluir", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        visita_id: encerramento.id, cliente_id: encerramento.clienteId, oportunidade_id: encerramento.oportunidadeId || null,
        usuario_id: String(usuario.id), descricao: descricao(objetivoDa(encerramento.descricao), resultado, desfecho, proxima), proxima_acao: proxima, proxima_data: proximaData,
      }) })
      const detalhe = await resposta.json().catch(() => ({})) as Registro
      if (!resposta.ok) throw new Error(texto(detalhe.detail) || `Falha ${resposta.status}`)
      setEncerramento(null); setSucesso("Visita concluída. Histórico e próxima ação sincronizados com o CTI."); await carregar()
    } catch (falha) { setErro(falha instanceof Error ? falha.message : "Não foi possível concluir a visita.") }
    finally { setSalvando(false) }
  }
...
function CampoSelect({ name, label, opcoes, valor, alterar }: { name: string; label: string; opcoes: { valor: string; texto: string }[]; valor?: string; alterar?: (valor: string) => void }) { return <label><span className="mb-2 block text-sm text-slate-300">{label}</span><select name={name} value={alterar ? valor : undefined} defaultValue={alterar ? undefined : valor} onChange={alterar ? (e) => alterar(e.target.value) : undefined} className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4"><option value="">Selecione</option>{opcoes.map((opcao) => <option key={`${name}-${opcao.valor}`} value={opcao.valor}>{opcao.texto}</option>)}</select></label> }
