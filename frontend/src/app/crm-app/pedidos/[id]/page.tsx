...
import { ArrowLeft, Building2, FileCheck2, Loader2, Mail, PackageCheck, Save } from "lucide-react"
...
export default function PedidoCrmAppPage() {
  ...
  return <main className="min-h-[100dvh] bg-[#020817] px-4 py-5 pb-24 text-white sm:px-6"><div className="mx-auto max-w-4xl">
    ...
    {dados && <div className="space-y-4">
      ...
      <section className="rounded-3xl border border-[#16325c] bg-[#07162b] p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-bold">Destinatários responsáveis</h3><p className="text-sm text-slate-400">Pessoas que deverão receber o pedido</p></div><div className="flex items-center gap-2"><span className="rounded-full border border-amber-700 px-3 py-1 text-xs text-amber-300">ENVIO {statusEnvio}</span><button type="button" onClick={() => setEditando((valor) => !valor)} className="rounded-xl border border-cyan-700 px-3 py-2 text-sm text-cyan-200">{destinatarios.length ? "Editar" : "Adicionar"}</button></div></div>
        ...
        {!editando && texto(dados.envio?.observacoes_envio, "") && <div className="mt-4 rounded-2xl bg-[#020817] p-4 text-sm text-slate-300"><strong className="block text-xs text-slate-500">Observações para o envio</strong>{texto(dados.envio?.observacoes_envio)}</div>}<p className="mt-4 text-xs text-amber-300">O pedido está gerado. O envio real por e-mail será executado somente quando o transporte estiver integrado e confirmado.</p></section>
