...
def _nome_valido(nome: str) -> bool:
    ...
    if re.fullmatch(r"[0-9]+(?:[.,][0-9]+)?", texto):
        return False
    return bool(re.search(r"[A-Za-zÀ-ÖØ-öø-ÿ]", texto))
...
def _nome_cliente(item: dict[str, Any]) -> str:
    return str(item.get("razao_social") or item.get("nome_fantasia") or item.get("nome") or item.get("cliente") or item.get("empresa") or "").strip()
...
def _criar_ou_atualizar_cliente(cliente: ClienteCreate | ClienteContexto) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    nome = payload["nome"]
    if not _nome_valido(str(nome)):
        raise HTTPException(status_code=422, detail="Informe um nome / razão social válido para o cliente.")
    ...
    if payload.get("cnpj"):
        try:
            ...
            candidatos = []
    if not candidatos:
        candidatos = supabase.table("clientes").select("*").ilike("nome", str(nome)).limit(1).execute().data or []
    if candidatos:
...
def _contexto_comercial(dados: ClienteOportunidadeCreate) -> dict[str, Any]:
    return {"linhas": dados.oportunidade.linha_equipamentos, "equipamentos": dados.oportunidade.equipamento, "municipio": dados.oportunidade.municipio or dados.cliente.cidade, "uf": dados.oportunidade.estado or dados.cliente.estado, "ddd": dados.cliente.ddd or dados.oportunidade.ddd, "sub_regiao": dados.cliente.sub_regiao or dados.oportunidade.sub_regiao, "segmento": dados.cliente.segmento or dados.cliente.categoria}
...
def criar_cliente_e_oportunidade(dados: ClienteOportunidadeCreate):
    ...
    try:
        ...
        etapa = "pipeline"
        _registrar_auxiliar("cti_pipeline", {"oportunidade_id": oportunidade_id, "etapa_anterior": None, "nova_etapa": "OPORTUNIDADE", "etapa": "OPORTUNIDADE", "usuario_id": oportunidade.responsavel_id, "observacao": "Primeira movimentação automática da oportunidade.", "data": agora.date().isoformat(), "hora": agora.time().replace(microsecond=0).isoformat()}, avisos, "pipeline")
        etapa = "historico"
        _registrar_auxiliar("cti_oportunidade_historico", {"oportunidade_id": oportunidade_id, "tipo": "OPORTUNIDADE", "descricao": "Oportunidade criada pelo App CRM.", "usuario_id": oportunidade.responsavel_id, "payload": {"oportunidade": oportunidade_criada, "contexto_comercial": contexto, "backend_version": CRM_APP_BACKEND_VERSION}, "created_at": _now()}, avisos, "histórico")
