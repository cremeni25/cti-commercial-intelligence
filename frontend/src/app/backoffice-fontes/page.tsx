...
import { Database, FileUp, Loader2, ShieldCheck, XCircle } from "lucide-react"
...
export default function BackofficeFontesPage() {
  ...
  async function enviar() {
    ...
    try {
      ...
      setMensagem(resposta.duplicado ? "Esta fonte já estava registrada. Nenhuma duplicação foi criada." : "Fonte recebida e original preservado. A publicação para IA permanece bloqueada até homologação.")
      setArquivo(null)
      await carregar()
    } catch (e) {
  ...
  async function rejeitar(fonte: Fonte) {
    if (fonte.status_governanca === "REJEITADO") return
    setErro("")
    try {
      await requisitar(`/${fonte.id}/governanca`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status: "REJEITADO", observacao: "Rejeitado pelo ADMIN_MASTER no Back Office Universal." }),
      })
      await carregar()
    } catch (e) {
      ...
      <section className="min-w-0 flex-1">
        ...
        <div className="mx-auto max-w-[1500px] p-4 sm:p-6 lg:p-8">
          ...
          <section className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ["Fontes registradas", dados.total],
              ["Aguardando interpretação", dados.por_status.RECEBIDO || 0],
              ["Homologadas", dados.por_status.HOMOLOGADO || 0],
              ["Publicadas para IA", publicados],
            ].map(([label, valor]) => (
              <div key={String(label)} className="min-w-0 rounded-2xl border border-[#13203f] bg-[#071427] p-4">
                <p className="text-xs uppercase tracking-wider text-slate-500">{label}</p>
                <p className="mt-2 text-2xl font-bold text-slate-100">{valor}</p>
              </div>
            ))}
          </section>
          ...
          <section className="mt-5 rounded-2xl border border-[#17345e] bg-[#071427] p-4 sm:p-5">
            ...
            <div className="mt-4 flex flex-col gap-3 lg:flex-row">
              <label className="flex min-h-12 flex-1 cursor-pointer items-center rounded-xl border border-dashed border-slate-600 bg-slate-950/40 px-4 text-sm text-slate-300 hover:border-cyan-600">
                <input className="hidden" type="file" onChange={(e: ChangeEvent<HTMLInputElement>) => setArquivo(e.target.files?.[0] || null)} />
                {arquivo ? `${arquivo.name} · ${tamanho(arquivo.size)}` : "Selecionar arquivo"}
              </label>
            ...
            {carregando ? <div className="flex items-center gap-2 p-6 text-sm text-slate-400"><Loader2 className="animate-spin" size={18} /> Carregando governança...</div> : dados.fontes.length === 0 ? <p className="p-6 text-sm text-slate-500">Nenhuma fonte registrada ainda.</p> : (
              <div className="overflow-x-auto">
                <table className="w-full min-w-[980px] text-left text-sm">
                  <thead className="bg-[#091a33] text-xs uppercase tracking-wider text-slate-500"><tr><th className="px-4 py-3">Fonte</th><th className="px-4 py-3">Tipo</th><th className="px-4 py-3">Governança</th><th className="px-4 py-3">Classificação</th><th className="px-4 py-3">IA</th><th className="px-4 py-3">Recebido</th><th className="px-4 py-3"></th></tr></thead>
                  <tbody>{dados.fontes.map((fonte) => (
                    <tr key={fonte.id} className="border-t border-[#13203f] align-top">
                      <td className="px-4 py-4"><p className="max-w-[320px] break-words font-medium text-slate-200">{fonte.nome_arquivo}</p><p className="mt-1 text-xs text-slate-500">{tamanho(fonte.tamanho_bytes)} · SHA {fonte.sha256.slice(0, 10)}…</p></td>
                      <td className="px-4 py-4 text-slate-300">{fonte.tipo_detectado}</td>
                      <td className="px-4 py-4"><span className={`rounded-full border px-2.5 py-1 text-xs ${fonte.status_governanca === "REJEITADO" ? "border-red-900 bg-red-950/30 text-red-300" : "border-cyan-900 bg-cyan-950/30 text-cyan-300"}`}>{fonte.status_governanca}</span><div className="mt-2 flex gap-1">{ETAPAS.map((etapa) => <span key={etapa} title={etapa} className={`h-1.5 w-5 rounded-full ${ETAPAS.indexOf(etapa) <= ETAPAS.indexOf(fonte.status_governanca) ? "bg-cyan-400" : "bg-slate-800"}`} />)}</div></td>
                      <td className="px-4 py-4 text-slate-400">{fonte.classificacao_negocio}</td>
                      ...
                      <td className="px-4 py-4"><button disabled={fonte.status_governanca === "REJEITADO"} onClick={() => void rejeitar(fonte)} className="flex items-center gap-1 text-xs text-red-300 disabled:opacity-30"><XCircle size={15} /> Rejeitar</button></td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
