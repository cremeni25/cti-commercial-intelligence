...
type Data = {
  ...
  rankings: Record<string, Rank[]>; serie_temporal: Serie[]
  clientes_sem_registro_recente?: Array<{ nome: string; dias_sem_registro: number | null }>
  heatmap: Array<{ regiao: string; uf: string }>; drilldown: Record<string, Rank[]>; empty_state: string | null
}
...
function Card({title,value,compare}:{title:string;value:string;compare?:{percentual:number;direcao:string}}){return <div className="rounded-xl border bg-white p-5 shadow-sm"><p className="text-sm text-slate-500">{title}</p><p className="mt-2 text-2xl font-semibold">{value}</p>{compare&&<p className="mt-2 text-xs text-slate-500">{compare.percentual>0?"+":""}{compare.percentual}% vs. comparação</p>}</div>}
function Bars({items}:{items:Rank[]}){const max=Math.max(1,...items.map(i=>i.quantidade));return <div className="space-y-3">{items.slice(0,10).map(i=><div key={i.nome}><div className="flex justify-between text-xs"><span className="truncate pr-2">{i.nome}</span><strong>{i.quantidade}</strong></div><div className="mt-1 h-2 rounded bg-slate-100"><div className="h-2 rounded bg-blue-600" style={{width:`${Math.max(3,i.quantidade/max*100)}%`}}/></div></div>)}</div>}
...
export default function InteligenciaPage(){
  ...
  return <main className="min-h-screen bg-slate-50 p-4 md:p-8"><div className="mx-auto max-w-7xl space-y-6">
    <header><p className="text-sm font-medium uppercase tracking-wide text-blue-700">CTI · mercado realizado</p><h1 className="text-3xl font-bold">Inteligência de Mercado</h1><p className="mt-2 text-slate-600">ANFIR e fatos históricos já ocorridos. Este painel não representa Funil, pipeline, conversão do CRM ou oportunidades abertas.</p></header>
    <section className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><strong>Separação obrigatória:</strong> ANFIR = realizado. CRM = operação comercial. Funil = ciclo das oportunidades. Entre eles existe correlação analítica, nunca fusão de registros.</section>
    ...
    {loading&&<div className="rounded-xl border bg-white p-10 text-center">Carregando inteligência de mercado...</div>}{error&&<div className="rounded-xl border border-red-200 bg-red-50 p-6 text-red-800">{error}</div>}{!loading&&data?.empty_state&&<div className="rounded-xl border bg-white p-10 text-center">{data.empty_state}</div>}
    {!loading&&data&&!data.empty_state&&<><section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Card title="Registros realizados" value={data.kpis.volume.toLocaleString("pt-BR")} compare={data.kpis.comparacoes.volume}/><Card title="Valor registrado" value={moeda(data.kpis.valor)} compare={data.kpis.comparacoes.valor}/><Card title="Ticket médio histórico" value={moeda(data.kpis.ticket_medio)} compare={data.kpis.comparacoes.ticket_medio}/><Card title="Clientes históricos" value={String(data.resumo?.clientes_unicos??0)} /></section>
      <section className="grid gap-6 lg:grid-cols-3"><div className="rounded-xl border bg-white p-5"><h2 className="font-semibold">Evolução do mercado realizado</h2><div className="mt-4 space-y-2 text-sm">{data.serie_temporal.slice(-12).map(i=><div key={i.periodo} className="grid grid-cols-3 gap-2 border-b pb-2"><span>{i.periodo}</span><span>{i.volume} registro(s)</span><span>{moeda(i.valor)}</span></div>)}</div></div><div className="rounded-xl border bg-white p-5"><h2 className="font-semibold">Distribuição por UF</h2><div className="mt-4"><Bars items={data.rankings.uf||[]}/></div></div><div className="rounded-xl border bg-white p-5"><h2 className="font-semibold">Distribuição por produto</h2><div className="mt-4"><Bars items={data.rankings.produto||[]}/></div></div></section>
      <section className="grid gap-6 lg:grid-cols-2"><div className="rounded-xl border bg-white p-5"><h2 className="font-semibold">Recorrência histórica</h2><p className="mt-2 text-sm text-slate-500">Dias desde o último registro realizado encontrado. Não significa oportunidade perdida nem ausência de compra no CRM.</p><div className="mt-4 max-h-64 overflow-auto text-sm">{(data.clientes_sem_registro_recente||[]).map(c=><div key={c.nome} className="flex justify-between border-b py-2"><span>{c.nome}</span><strong>{c.dias_sem_registro??0} dias</strong></div>)}</div></div><div className="rounded-xl border bg-white p-5"><h2 className="font-semibold">Heat map do mercado realizado</h2><div className="mt-4 space-y-2 text-sm">{heat.map(([k,v])=><div key={k} className="flex justify-between rounded bg-slate-100 p-2"><span>{k}</span><strong>{v}</strong></div>)}</div></div></section>
      <section className="rounded-xl border bg-white p-5"><h2 className="font-semibold">Drill-down de mercado</h2><div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{dims.map(d=><div key={d}><h3 className="mb-2 text-sm font-medium capitalize">{d}</h3><div className="max-h-52 overflow-auto text-xs">{(data.drilldown[d]||[]).slice(0,15).map(i=><div key={i.nome} className="flex justify-between border-b py-2"><span className="truncate pr-2">{i.nome}</span><strong>{i.quantidade}</strong></div>)}</div></div>)}</div></section>
      <section className="rounded-xl border bg-white p-4 text-sm text-slate-600">Origem: {data.metadata.origem} · Natureza: {data.metadata.natureza_dados||"FATO_MERCADO_REALIZADO"} · Contexto: {data.metadata.contexto_operacional} · Atualização: {new Date(data.metadata.ultima_atualizacao).toLocaleString("pt-BR")}</section></>}
  </div></main>
}
