        ...
        <div className="p-8 space-y-8">
          ...
          <section className="rounded-2xl bg-[#091a33] border border-[#13203f] p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div>
                <h2 className="text-2xl font-bold text-white">Dados operacionais</h2>
                <p className="text-gray-400">{estados.size} estados encontrados nos registros CTI.</p>
              </div>
              <input
                value={busca}
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="border-b border-[#13203f] text-gray-400">
                      ...
                      <th className="p-3">Estados</th>
                      <th className="p-3">Linhas</th>
                    </tr>
                  </thead>
                  <tbody>
                    {lista.map((item) => (
                      <tr key={item.nome} className="border-b border-[#13203f] text-gray-200">
                        <td className="p-3 font-semibold">{item.nome}</td>
                        <td className="p-3">{item.quantidade_registros}</td>
                        <td className="p-3">R$ {(item.valor_total ?? 0).toLocaleString("pt-BR")}</td>
                        <td className="p-3">{item.estados?.join(", ") || "-"}</td>
                        <td className="p-3">{item.linhas?.join(", ") || "-"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
