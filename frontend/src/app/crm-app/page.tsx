...
const textos = {
  "pt-BR": {
    brand: "CTI CRM · campo", hello: "Olá, {name}", subtitle: "Seu dia comercial em uma tela. Escolha a próxima ação e deixe o CTI cuidar do restante.", today: "Meu dia",
    visits: "Visitas hoje", pending: "Pendências", openDeals: "Negócios abertos", mainAction: "Nova ação", mainActionHelp: "Visita, contato, proposta ou continuidade da negociação.",
    next: "Continuar negociação", nextHelp: "Abra sua carteira e avance exatamente de onde parou.", agenda: "Ver pendências", agendaHelp: "Retornos, compromissos e atividades que precisam de atenção.",
    clients: "Buscar cliente", clientsHelp: "Encontre o cliente e parta dele para a próxima ação.", current: "Negociação em destaque", noCurrent: "Nenhuma negociação aberta no momento.",
    details: "Abrir negócios", more: "Mais recursos", moreHelp: "Recursos completos continuam disponíveis quando você precisar.", syncError: "Não foi possível sincronizar o CRM agora.",
    online: "Online", reconnecting: "Reconectando", activities: "Atividades", pipeline: "Pipeline", proposals: "Propostas", orders: "Pedidos", sales: "Vendas", ai: "IA Comercial", financial: "Controle financeiro", logout: "Sair", user: "Usuário",
  },
  en: {
    brand: "CTI CRM · field", hello: "Hello, {name}", subtitle: "Your sales day on one screen. Choose the next action and let CTI handle the rest.", today: "My day",
    visits: "Visits today", pending: "Pending", openDeals: "Open deals", mainAction: "New action", mainActionHelp: "Visit, contact, proposal or continue a deal.",
    next: "Continue deal", nextHelp: "Open your portfolio and continue exactly where you left off.", agenda: "View pending", agendaHelp: "Follow-ups, appointments and activities requiring attention.",
    clients: "Find account", clientsHelp: "Find the account and start the next action from there.", current: "Highlighted deal", noCurrent: "No open deal right now.",
    details: "Open deals", more: "More resources", moreHelp: "Full CRM resources remain available whenever needed.", syncError: "CRM could not synchronize right now.",
    online: "Online", reconnecting: "Reconnecting", activities: "Activities", pipeline: "Pipeline", proposals: "Proposals", orders: "Orders", sales: "Sales", ai: "Sales AI", financial: "Financial control", logout: "Sign out", user: "User",
  },
  es: {
    brand: "CTI CRM · campo", hello: "Hola, {name}", subtitle: "Tu día comercial en una sola pantalla. Elige la próxima acción y deja que CTI gestione el resto.", today: "Mi día",
    visits: "Visitas hoy", pending: "Pendientes", openDeals: "Negocios abiertos", mainAction: "Nueva acción", mainActionHelp: "Visita, contacto, propuesta o continuidad del negocio.",
    next: "Continuar negocio", nextHelp: "Abre tu cartera y continúa exactamente donde la dejaste.", agenda: "Ver pendientes", agendaHelp: "Retornos, compromisos y actividades que requieren atención.",
    clients: "Buscar cliente", clientsHelp: "Encuentra al cliente y parte desde él hacia la próxima acción.", current: "Negocio destacado", noCurrent: "No hay negocios abiertos en este momento.",
    details: "Abrir negocios", more: "Más recursos", moreHelp: "Los recursos completos siguen disponibles cuando los necesites.", syncError: "No fue posible sincronizar el CRM ahora.",
    online: "Online", reconnecting: "Reconectando", activities: "Actividades", pipeline: "Pipeline", proposals: "Propuestas", orders: "Pedidos", sales: "Ventas", ai: "IA Comercial", financial: "Control financiero", logout: "Salir", user: "Usuario",
  },
...
function lista(payload: unknown): Registro[] {
  ...
  if (payload && typeof payload === "object") {
    ...
    for (const chave of ["dados", "itens", "oportunidades", "resultado", "atividades", "clientes", "propostas", "pedidos", "vendas"]) {
...
async function carregar(path: string) {
  const resposta = await fetchCrmSeguroProxy(path, { cache: "no-store" })
  const payload = await resposta.json().catch(() => null)
  if (!resposta.ok) {
    const detalhe = payload && typeof payload === "object" && "detail" in payload ? String((payload as Registro).detail || "") : ""
    throw new Error(detalhe || `HTTP ${resposta.status}`)
  }
  return payload
}
...
export default function CrmAppPage() {
  ...
  const sincronizar = useCallback(async () => {
    ...
    try {
      const resultados = await Promise.allSettled([
        carregar("crm-seguro/nucleo-comercial"),
        carregar("crm-seguro/agenda"),
        carregar("crm-seguro/atividades"),
        carregar("crm-seguro/clientes"),
        carregar("crm-seguro/propostas"),
        carregar("crm-seguro/pedidos"),
        carregar("crm-seguro/vendas"),
      ])
      const [nucleoR, agendaR, atividadesR, clientesR, propostasR, pedidosR, vendasR] = resultados
      if (nucleoR.status === "rejected") throw nucleoR.reason
      ...
      setOnline(true)
      if (resultados.some((r) => r.status === "rejected")) setAviso(t.syncError)
    } catch {
      setOnline(false)
  ...
  const nome = usuario?.nome?.trim() || usuario?.email?.trim() || t.user
  const primeiroNome = nome.includes("@") ? nome : nome.split(" ")[0]
      ...
      <header className="sticky top-0 z-20 border-b border-cyan-950/80 bg-[#061126]/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex w-full max-w-3xl items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">{nome}{usuario?.tipo_usuario ? ` · ${usuario.tipo_usuario.replaceAll("_", " ")}` : ""}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => void sair()} title={t.logout} aria-label={t.logout} className="grid size-9 place-items-center rounded-full border border-slate-700 bg-slate-900 text-slate-300"><LogOut size={16}/></button>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto w-full max-w-3xl px-4 py-5 sm:px-6">
        ...
        <section className="rounded-3xl border border-[#16325c] bg-gradient-to-br from-[#0a2242] to-[#07162b] p-5 shadow-xl">
          <div className="flex items-start justify-between gap-3">
            <div><h2 className="text-3xl font-bold">{hello}</h2><p className="mt-2 text-base leading-6 text-slate-300">{t.subtitle}</p></div>
            <button type="button" onClick={() => void sincronizar()} aria-label="Sincronizar CRM" className="grid size-12 shrink-0 place-items-center rounded-2xl border border-cyan-800 bg-cyan-950/30 text-cyan-300"><RefreshCw size={20} className={sincronizando ? "animate-spin" : ""}/></button>
          </div>
        ...
        <section className="mt-5 grid gap-3">
          <Link href="/crm-app/acao" className="flex min-h-28 items-center gap-4 rounded-3xl bg-cyan-500 p-5 text-slate-950 shadow-xl active:scale-[.99]">
            ...
            <span className="min-w-0 flex-1"><span className="block text-2xl font-black">{t.mainAction}</span><span className="mt-1 block text-sm font-medium text-slate-900/75">{t.mainActionHelp}</span></span><ChevronRight size={25}/>
          </Link>
        ...
        <details className="mt-5 rounded-3xl border border-[#16325c] bg-[#061126] p-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">{recursos.map(({ href, label, valor, icon: Icon }) => <Link key={href} href={href} className="rounded-2xl border border-[#16325c] bg-[#091a33] p-4"><div className="flex items-center justify-between gap-2"><Icon size={20} className="text-cyan-300"/><strong className="text-sm text-cyan-200">{valor}</strong></div><span className="mt-3 block text-sm font-semibold">{label}</span></Link>)}</div>
        </details>
      </div>
    </main>
  )
