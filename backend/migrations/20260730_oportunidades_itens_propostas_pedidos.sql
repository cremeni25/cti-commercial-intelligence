...
create table if not exists public.cti_destinatarios_carrier (
  ...
  email text not null,
  regiao text,
  linhas_produto text[] not null default '{}',
  recebe_oportunidades boolean not null default false,
  recebe_propostas boolean not null default false,
  recebe_pedidos boolean not null default true,
  copia_obrigatoria boolean not null default false,
  ativo boolean not null default true,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (lower(email))
