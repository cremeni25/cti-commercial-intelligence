...
import ReconciliacaoFontePanel from "./ReconciliacaoFontePanel"
...
type Fonte = {
  ...
  metadados?: {
    destino_ingestao?: {
      ...
      regra?: string
      consumivel_dashboard?: boolean
      consumivel_ia?: boolean
    }
  } | null
  created_at: string
}
...
function candidatoOperacional(fonte: Fonte) {
  return fonte.metadados?.destino_ingestao?.destino === "CANDIDATO_OPERACIONAL_VALIDACAO"
}
...
export default function BackofficeFontesPage() {
  ...
  const [fonteReconciliacao, setFonteReconciliacao] = useState<Fonte | null>(null)
  ...
  async function semantica(fonte: Fonte) {
    ...
    try {
      ...
      setMensagem(`Semântica gerada: ${resposta.total_registros_semanticos} registro(s). Revise o preview e o destino antes de qualquer homologação ou promoção.`)
      await carregar()
  ...
  async function publicar(fonte: Fonte) {
    ...
    try {
      ...
      setMensagem(`Fonte ${fonte.nome_arquivo} publicada para a IA Comercial. O conhecimento semântico permanece separado dos registros operacionais.`)
      await carregar()
      setPreview(await requisitar(`/${fonte.id}/preview`))
        ...
        <div className="mx-auto max-w-[1500px] p-4 sm:p-6 lg:p-8">
          ...
          <section className="mt-5 overflow-hidden rounded-2xl border border-[#13203f] bg-[#071427]">
            ...
            {carregando ? <div className="flex items-center gap-2 p-6 text-sm text-slate-400"><Loader2 className="animate-spin" size={18} /> Carregando governança...</div> : dados.fontes.length === 0 ? <p className="p-6 text-sm text-slate-500">Nenhuma fonte registrada ainda.</p> : (
              <div className="overflow-x-auto"><table className="w-full min-w-[1320px] text-left text-sm"><thead className="bg-[#091a33] text-xs uppercase tracking-wider text-slate-500"><tr><th className="px-4 py-3">Fonte</th><th className="px-4 py-3">Tipo</th><th className="px-4 py-3">Interpretação</th><th className="px-4 py-3">Semântica</th><th className="px-4 py-3">Destino</th><th className="px-4 py-3">Governança</th><th className="px-4 py-3">IA</th><th className="px-4 py-3">Ações</th></tr></thead>
                <tbody>{dados.fontes.map((fonte) => { const ocupada = acaoId === fonte.id; const destino = fonte.metadados?.destino_ingestao; return <tr key={fonte.id} className="border-t border-[#13203f] align-top">
                  ...
                  <td className="max-w-[220px] px-4 py-4 text-xs leading-5">{destino?.destino ? <><span className={destino.destino === "CANDIDATO_OPERACIONAL_VALIDACAO" ? "text-amber-300" : destino.destino === "CONHECIMENTO_SEMANTICO" ? "text-violet-300" : "text-slate-300"}>{destino.destino}</span><p className="mt-1 text-slate-500">{destino.classificacao || "—"}</p></> : <span className="text-slate-600">Aguardando classificação</span>}</td>
                  ...
                  <td className="px-4 py-4"><div className="flex max-w-[420px] flex-wrap gap-2">
                    ...
                    {candidatoOperacional(fonte) && <button disabled={ocupada} onClick={() => setFonteReconciliacao(fonte)} className="flex items-center gap-1 rounded-lg border border-amber-700 px-2.5 py-1.5 text-xs text-amber-300 disabled:opacity-40"><GitMerge size={14} /> Reconciliar</button>}
