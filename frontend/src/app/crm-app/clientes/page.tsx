...
export default function ClientesCrmAppPage() {
  ...
  const filtrados=useMemo(()=>{const termo=busca.trim().toLocaleLowerCase("pt-BR");if(termo)return clientes.filter((item)=>`${item.nome} ${item.cidade} ${item.estado}`.toLocaleLowerCase("pt-BR").includes(termo));if(letra)return clientes.filter((item)=>inicial(item.nome)===letra);return []},[busca,clientes,letra])
  ...
  return <main className="min-h-[100dvh] bg-[#020817] px-4 py-5 pb-24 text-white sm:px-6"><div className="mx-auto max-w-5xl">
    ...
    <label className="relative mb-4 block"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18}/><input value={busca} onChange={(e)=>{setBusca(e.target.value);if(e.target.value)setLetra("")}} placeholder="Buscar cliente ou cidade" className="h-12 w-full rounded-2xl border border-[#16325c] bg-[#07162b] pl-11 pr-11"/>{busca&&<button type="button" onClick={()=>setBusca("")} className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg p-2 text-slate-400"><X size={18}/></button>}</label>

    {!carregando&&!erro&&<section className="mb-5 rounded-3xl border border-[#16325c] bg-[#07162b] p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><h2 className="font-semibold">Índice alfabético</h2><p className="text-xs text-slate-400">Escolha uma letra para listar somente os clientes correspondentes.</p></div>{letra&&<button type="button" onClick={()=>setLetra("")} className="text-sm font-semibold text-cyan-300">Limpar</button>}</div><div className="grid grid-cols-7 gap-2 sm:grid-cols-13">{letras.map((l)=>{const quantidade=contagemLetras.get(l)||0;return <button key={l} type="button" disabled={!quantidade} onClick={()=>{setLetra(l);setBusca("")}} className={`rounded-xl border px-2 py-2 text-sm font-bold ${letra===l?"border-cyan-400 bg-cyan-500 text-slate-950":"border-[#24466f] bg-[#020817] text-slate-300 disabled:opacity-25"}`}>{l}<span className="ml-1 text-[10px] font-normal">{quantidade||""}</span></button>})}</div></section>}

    {erro&&<div className="rounded-2xl border border-red-900 bg-red-950/40 p-4 text-red-200">{erro}</div>}
    {carregando?<div className="grid min-h-64 place-items-center gap-3 text-slate-400"><Loader2 className="animate-spin text-cyan-300"/><span>Carregando carteira comercial...</span></div>:erro?null:!modoBusca&&!modoLetra?<div className="grid min-h-64 place-items-center rounded-3xl border border-dashed border-[#24466f] p-8 text-center"><div><Building2 className="mx-auto mb-3 text-cyan-300"/><p className="font-semibold">Selecione uma letra ou use a busca</p><p className="mt-1 text-sm text-slate-400">A carteira completa permanece disponível sem carregar centenas de cartões de uma só vez.</p></div></div>:filtrados.length===0?<div className="grid min-h-64 place-items-center rounded-3xl border border-dashed border-[#24466f] p-8 text-center"><div><AlertCircle className="mx-auto mb-3 text-cyan-300"/><p className="font-semibold">Nenhum cliente encontrado</p><Link href="/crm-app/clientes/nova" className="mt-3 inline-flex text-sm font-semibold text-cyan-300">Cadastrar novo cliente</Link></div></div>:<><div className="mb-3 flex items-center justify-between text-sm text-slate-400"><span>{filtrados.length} {filtrados.length===1?"cliente encontrado":"clientes encontrados"}{modoLetra?` na letra ${letra}`:""}</span>{modoLetra&&<button type="button" onClick={()=>setLetra("")} className="font-semibold text-cyan-300">Voltar ao alfabeto</button>}</div><div className="grid gap-4 md:grid-cols-2">{filtrados.map((cliente)=><Link key={cliente.chave} href={`/crm-app/clientes/${encodeURIComponent(cliente.id)}?nome=${encodeURIComponent(cliente.nome)}`} className="group rounded-3xl border border-[#16325c] bg-[#07162b] p-5 transition hover:border-cyan-700"><div className="flex items-center gap-3"><span className="rounded-2xl bg-cyan-950/50 p-3 text-cyan-300"><Building2 size={22}/></span><div className="min-w-0 flex-1"><h2 className="truncate font-bold">{cliente.nome}</h2><p className="text-xs text-slate-400">{cliente.cidade?`${cliente.cidade}${cliente.estado?`/${cliente.estado}`:""}`:"Cadastro comercial"}</p><p className="mt-2 text-xs font-semibold text-cyan-300">{cliente.negocios} {cliente.negocios===1?"negociação":"negociações"} · abrir dossiê</p></div><ChevronRight size={20} className="text-cyan-300"/></div></Link>)}</div></>}
  </div></main>
}
