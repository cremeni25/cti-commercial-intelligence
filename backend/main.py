...
# ============================================================
# ANFIR ENGINE V2 — LEITURA TOTAL (MULTI-BLOCO REAL)
# ============================================================
...
def normalizar_anfir_100(contents):
    ...
    for i in range(len(df)):
        ...
        # ====================================================
        # DETECTA LINHA COM MÊS
        # ====================================================

        mes_detectado = None

        for item in linha_limpa:

            if item in mapa_meses:
                mes_detectado = mapa_meses[item]
                break
        ...
        while j < len(df):
            ...
            row_limpa = [limpar_texto(x) for x in row]

            # para se encontrar novo mês
            if any(x in mapa_meses for x in row_limpa):
                break
            ...
            try:
                ...
                for val in row:
                    ...
                    elif "LTDA" in texto or "LOG" in texto:
                        cliente = texto

                    elif any(x in texto for x in ["BAU", "FRIG", "CARRETA", "SIDER"]):
                        segmento = texto

                    elif any(x in texto for x in ["IBIPORA", "RANDON", "FACCHINI"]):
                        fabricante = texto

                    elif len(texto) > 3 and not municipio:
                        municipio = texto
                ...
                # anti duplicação
                chave = f"{mes_detectado}_{estado}_{municipio}_{valor}"
                h = hashlib.md5(chave.encode()).hexdigest()

                if h not in registros_hash:
                    registros_hash.add(h)
                    registros.append(registro)
