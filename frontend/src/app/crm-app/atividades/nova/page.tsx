...
export default function NovaAtividadePage() {
  ...
  useEffect(()=>{let ativo=true; void (async()=>{setCarregando(true);setErro("");try{
    ...
    if(!ativo)return
    const listaClientes=(Array.isArray(clientesDados)?clientesDados:[]).map((item:Registro)=>({id:texto(item.id),nome:texto(item.nome||item.razao_social||item.nome_fantasia),cidade:texto(item.cidade||item.municipio),estado:texto(item.estado||item.uf)})).filter((item)=>item.id&&item.nome)
    setClientes(listaClientes.sort((a,b)=>a.nome.localeCompare(b.nome,"pt-BR")))
    setNegociacoes((Array.isArray(nucleoDados)?nucleoDados:[]).map((item:Registro)=>({oportunidade_id:texto(item.oportunidade_id),cliente_id:texto(item.cliente_id),cliente_nome:texto(item.cliente_nome),titulo:texto(item.titulo)||"Oportunidade comercial",etapa:texto(item.etapa)||"OPORTUNIDADE",proposta_id:texto(item.proposta_id)||null,proposta_numero:texto(item.proposta_numero)||null,pedido_id:texto(item.pedido_id)||null,pedido_numero:texto(item.pedido_numero)||null,encerrada:Boolean(item.encerrada)})).filter((item)=>item.oportunidade_id))
  ...
  return <main className="min-h-[100dvh] bg-[#020817] px-4 py-5 text-white sm:px-6"><div className="mx-auto max-w-3xl">
    ...
    {carregando?<div className="grid min-h-72 place-items-center"><Loader2 className="animate-spin text-cyan-300"/></div>:<form onSubmit={salvar} className="grid gap-4 rounded-3xl border border-[#16325c] bg-[#07162b] p-5 sm:grid-cols-2">
      ...
      {cliente&&<div className="sm:col-span-2 rounded-xl border border-emerald-900 bg-emerald-950/20 p-3 text-sm text-emerald-200">Cliente selecionado: <strong>{cliente.nome}</strong></div>}
