  ...
  return <section className="rounded-3xl border border-cyan-500/30 bg-[#061126] p-5 sm:p-6">
    <header className="flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <p className="mt-2 text-xs text-cyan-300">Indicador observado na ANFIR; não é market share contábil definitivo. Totais e linhas em azul podem ser abertos para rastreabilidade. A concorrência por fabricante é apresentada na seção de Inteligência Competitiva abaixo.</p>
      </div>
    ...
    {tab==="viena"&&<div className="mt-6 space-y-6">
      ...
      <Table headers={["Segmento","Mercado","Carrier","Carrier %","Sem contato","Não participou","Q2 vs Q1"]} rows={v.segmentos.map(s=>{const familia=FAMILY_SLUG[s.codigo];return [cellLink(s.segmento,drill({titulo:`${s.segmento} · mercado ANFIR 2026`,familia})),cellLink(formatNumber(s.mercado),drill({titulo:`${s.segmento} · mercado ANFIR 2026`,familia})),cellLink(formatNumber(s.carrier),drill({titulo:`${s.segmento} · Carrier observada`,familia,campo:"categoria",valor:"CARRIER"})),`${s.carrier_percentual_observado.toFixed(1)}%`,cellLink(formatNumber(s.sem_contato),drill({titulo:`${s.segmento} · Sem contato`,familia,campo:"categoria",valor:"SEM_CONTATO"})),cellLink(formatNumber(s.nao_participou),drill({titulo:`${s.segmento} · Não participamos da proposta`,familia,campo:"causa",valor:"COBERTURA_COMERCIAL"})),s.q2_vs_q1_percentual===null?"—":`${s.q2_vs_q1_percentual>0?"+":""}${s.q2_vs_q1_percentual.toFixed(1)}%`]})}/>
