...
function lista(payload: unknown): Registro[] {
  ...
  if (payload && typeof payload === "object") {
    ...
    for (const k of ["dados", "itens", "resultado", "atividades", "oportunidades"]) if (Array.isArray(objeto[k])) return objeto[k] as Registro[]
  }
  return []
}
...
function descricaoObjetivo(objetivo: string) { return `[OBJETIVO]\n${objetivo.trim() || "Não informado"}` }
...
export default function VisitasPage() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      const [atividadesResposta, clientesResposta, oportunidadesResposta] = await Promise.all([
        ...
        fetchCrmSeguroProxy("crm-seguro/clientes", { cache: "no-store" }),
      ...
      if (!clientesResposta.ok) throw new Error(`Não foi possível carregar os clientes (${clientesResposta.status}).`)
      if (!oportunidadesResposta.ok) throw new Error(`Não foi possível carregar os negócios (${oportunidadesResposta.status}).`)
      const atividadesPayload = await atividadesResposta.json()
      const clientesPayload = await clientesResposta.json()
      const oportunidadesPayload = await oportunidadesResposta.json()
      ...
      setVisitas(lista(atividadesPayload).filter((item) => chave(item.tipo || item.tipo_atividade).includes("VISITA")).map((item) => {
        const idCliente = texto(item.cliente_id), idOportunidade = texto(item.oportunidade_id)
        return {
          id: texto(item.id || item.atividade_id), clienteId: idCliente,
          cliente: texto(item.cliente_nome || item.cliente) || nomesClientes.get(idCliente) || "Cliente não identificado",
          oportunidadeId: idOportunidade, oportunidade: texto(item.oportunidade_titulo) || nomesOportunidades.get(idOportunidade) || "",
          titulo: texto(item.titulo || item.assunto) || "Visita comercial", descricao: texto(item.descricao),
      ...
      }).filter((item) => item.id).sort((a, b) => `${b.data}${b.horario}`.localeCompare(`${a.data}${a.horario}`)))
    } catch (falha) { setErro(falha instanceof Error ? falha.message : "Não foi possível carregar as visitas.") }
  ...
  const visiveis = useMemo(() => visitas.filter((item) => {
    const termo = busca.trim().toLocaleLowerCase("pt-BR"), e = estado(item)
  ...
  const clientesFiltrados = useMemo(() => clienteId || !clienteBusca.trim() ? [] : clientes.filter((item) => chave(`${item.nome} ${item.cidade} ${item.uf}`).includes(chave(clienteBusca))).slice(0, 8), [clienteBusca, clienteId, clientes])
  ...
  const oportunidadesFiltradas = useMemo(() => oportunidadeId || !oportunidadeBusca.trim() ? [] : oportunidadesDoCliente.filter((item) => chave(`${item.titulo} ${item.cliente}`).includes(chave(oportunidadeBusca))).slice(0, 8), [oportunidadeBusca, oportunidadeId, oportunidadesDoCliente])
  ...
  async function criarVisita(evento: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      evento.currentTarget.reset(); setNovaAberta(false); setClienteId(""); setClienteBusca(""); setOportunidadeId(""); setOportunidadeBusca(""); setSucesso("Visita agendada."); await carregar()
    } catch (falha) { setErro(falha instanceof Error ? falha.message : "Não foi possível agendar a visita.") }
    finally { setSalvando(false) }
  }

  async function atualizarStatus(visita: Visita, status: string) {
    setSalvando(true); setErro("")
    const resposta = await fetch(`/api/crm-proxy/crm/atividades/${encodeURIComponent(visita.id)}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status }) })
    const detalhe = await resposta.json().catch(() => ({})) as Registro
    if (!resposta.ok) { setSalvando(false); throw new Error(texto(detalhe.detail) || `Falha ${resposta.status}`) }
    await carregar(); setSalvando(false)
  }
  async function confirmarRealizada(visita: Visita) {
    try { await atualizarStatus(visita, "CONCLUIDA"); if (visita.oportunidadeId) router.push(`/crm-app/oportunidades/${encodeURIComponent(visita.oportunidadeId)}`); else setDecisao(visita) }
    catch (falha) { setErro(falha instanceof Error ? falha.message : "Não foi possível confirmar a visita.") }
  }
  async function confirmarNaoRealizada(visita: Visita) {
    try { await atualizarStatus(visita, "CANCELADA"); setSucesso("Visita marcada como não realizada.") }
    catch (falha) { setErro(falha instanceof Error ? falha.message : "Não foi possível atualizar a visita.") }
  }
  ...
  return <main className="min-h-[100dvh] bg-[#020817] px-4 py-5 pb-24 text-white sm:px-6"><div className="mx-auto max-w-6xl">
    <header className="mb-5 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><Link href="/crm-app" className="grid size-11 place-items-center rounded-2xl border border-[#16325c] bg-[#091a33] text-cyan-300"><ArrowLeft size={20}/></Link><div><p className="text-xs uppercase tracking-[.24em] text-cyan-400">CTI CRM</p><h1 className="text-2xl font-bold">Visitas</h1><p className="text-sm text-slate-400">A visita é um fato. Se houver chance comercial, evolua o mesmo ciclo até proposta, pedido ou venda.</p></div></div><button onClick={() => setNovaAberta(true)} className="inline-flex min-h-11 items-center gap-2 rounded-2xl bg-cyan-500 px-4 font-bold text-slate-950"><Plus size={18}/>Agendar visita</button></header>
    ...
    {carregando ? <div className="grid min-h-64 place-items-center"><Loader2 className="animate-spin text-cyan-300"/></div> : visiveis.length === 0 ? <section className="rounded-3xl border border-dashed border-[#24466f] p-8 text-center"><MapPinned className="mx-auto text-cyan-300"/><h2 className="mt-3 text-lg font-bold">Nenhuma visita neste filtro</h2></section> : <div className="space-y-3">{visiveis.map((visita) => { const e = estado(visita), finalizada = e === "REALIZADA" || e === "NAO_REALIZADA"; return <article key={visita.id} className="rounded-3xl border border-[#16325c] bg-[#07162b] p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold text-cyan-300">{rotuloEstado(e)}</p><h2 className="mt-1 text-lg font-bold">{visita.cliente}</h2><p className="text-sm text-slate-300">{visita.titulo}</p>{visita.oportunidade&&<p className="mt-1 text-xs text-slate-400">Negócio: {visita.oportunidade}</p>}</div><div className="text-right text-xs text-slate-400"><span className="inline-flex items-center gap-1"><CalendarDays size={14}/>{dataBr(visita.data)}</span>{visita.horario&&<span className="mt-1 flex items-center justify-end gap-1"><Clock3 size={14}/>{visita.horario}</span>}</div></div>{visita.descricao&&<p className="mt-4 whitespace-pre-line rounded-2xl bg-[#020817]/70 p-3 text-sm leading-6 text-slate-300">{visita.descricao}</p>}<div className="mt-4 flex flex-wrap gap-2">{e === "A_CONFIRMAR"&&<><button disabled={salvando} onClick={() => void confirmarRealizada(visita)} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-3 text-sm font-bold"><CheckCircle2 size={16}/>Visita realizada</button><button disabled={salvando} onClick={() => void confirmarNaoRealizada(visita)} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-slate-600 px-3 text-sm font-semibold text-slate-200"><XCircle size={16}/>Não realizada</button></>}{visita.oportunidadeId&&<Link href={`/crm-app/oportunidades/${encodeURIComponent(visita.oportunidadeId)}`} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-cyan-700 px-3 text-sm font-semibold text-cyan-200"><BriefcaseBusiness size={16}/>Continuar negócio</Link>}{e === "REALIZADA"&&!visita.oportunidadeId&&visita.clienteId&&<button onClick={() => abrirOportunidade(visita)} className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-cyan-500 px-3 text-sm font-bold text-slate-950"><BriefcaseBusiness size={16}/>Evoluir para negócio</button>}{finalizada&&visita.clienteId&&<Link href={`/crm-app/clientes/${encodeURIComponent(visita.clienteId)}?nome=${encodeURIComponent(visita.cliente)}`} className="inline-flex min-h-11 flex-1 items-center justify-center rounded-xl border border-[#24466f] px-3 text-sm font-semibold">Ver cliente</Link>}</div></article> })}</div>}
  </div>
  {novaAberta&&<Modal titulo="Agendar visita" fechar={() => setNovaAberta(false)}><form onSubmit={criarVisita} className="grid gap-4 sm:grid-cols-2"><label className="relative sm:col-span-2"><span className="mb-2 block text-sm text-slate-300">Cliente</span><input value={clienteBusca} onChange={(e) => { setClienteBusca(e.target.value); setClienteId(""); setOportunidadeId(""); setOportunidadeBusca("") }} placeholder="Digite nome ou cidade" className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4"/>{clientesFiltrados.length>0&&<div className="absolute z-30 mt-1 w-full overflow-hidden rounded-2xl border border-[#24466f] bg-[#07162b] shadow-2xl">{clientesFiltrados.map((item)=><button type="button" key={item.id} onClick={() => { setClienteId(item.id); setClienteBusca(item.nome) }} className="block min-h-12 w-full border-b border-[#16325c] px-4 text-left last:border-0"><strong>{item.nome}</strong><span className="ml-2 text-xs text-slate-400">{[item.cidade,item.uf].filter(Boolean).join("/")}</span></button>)}</div>}</label><label className="relative sm:col-span-2"><span className="mb-2 block text-sm text-slate-300">Negócio existente (opcional)</span><input disabled={!clienteId} value={oportunidadeBusca} onChange={(e) => { setOportunidadeBusca(e.target.value); setOportunidadeId("") }} placeholder={clienteId?"Digite o nome do negócio":"Selecione o cliente primeiro"} className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4 disabled:opacity-50"/>{oportunidadesFiltradas.length>0&&<div className="absolute z-30 mt-1 w-full overflow-hidden rounded-2xl border border-[#24466f] bg-[#07162b] shadow-2xl">{oportunidadesFiltradas.map((item)=><button type="button" key={item.id} onClick={() => { setOportunidadeId(item.id); setOportunidadeBusca(item.titulo) }} className="block min-h-12 w-full border-b border-[#16325c] px-4 text-left last:border-0"><strong>{item.titulo}</strong></button>)}</div>}</label><Campo name="titulo" label="Objetivo resumido"/><Campo name="data" label="Data" type="date" valor={hoje()}/><Campo name="horario" label="Horário" type="time"/><label className="sm:col-span-2"><span className="mb-2 block text-sm text-slate-300">Contexto da visita</span><textarea name="objetivo" rows={4} required className="w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4 py-3"/></label><button disabled={salvando||!clienteId} className="sm:col-span-2 min-h-12 rounded-2xl bg-cyan-500 font-bold text-slate-950 disabled:opacity-50">Agendar</button></form></Modal>}
  {decisao&&<Modal titulo="Visita realizada" fechar={() => setDecisao(null)}><div className="space-y-4"><div className="rounded-2xl bg-[#020817] p-4"><p className="font-bold">{decisao.cliente}</p><p className="mt-1 text-sm text-slate-400">A visita já está registrada como realizada.</p></div><p className="text-sm leading-6 text-slate-300">Existe possibilidade comercial que precisa ser acompanhada?</p><div className="grid gap-2 sm:grid-cols-2"><button onClick={() => { setDecisao(null); setSucesso("Visita encerrada sem oportunidade comercial.") }} className="min-h-12 rounded-2xl border border-[#24466f] font-semibold">Não. Encerrar aqui</button><button onClick={() => abrirOportunidade(decisao)} className="min-h-12 rounded-2xl bg-cyan-500 font-bold text-slate-950">Sim. Evoluir negócio</button></div></div></Modal>}
  </main>
}
...
function Campo({ name, label, type = "text", valor }: { name: string; label: string; type?: string; valor?: string }) { return <label><span className="mb-2 block text-sm text-slate-300">{label}</span><input name={name} type={type} defaultValue={valor} required={name!=="horario"} className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4"/></label> }
function Modal({ titulo, fechar, children }: { titulo: string; fechar: () => void; children: React.ReactNode }) { return <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-slate-950/85 p-4"><section className="my-6 w-full max-w-2xl rounded-3xl border border-[#24466f] bg-[#07162b] p-5 shadow-2xl"><div className="mb-5 flex items-center justify-between gap-3"><h2 className="text-xl font-bold">{titulo}</h2><button type="button" onClick={fechar} className="grid size-10 place-items-center rounded-xl border border-[#24466f]">×</button></div>{children}</section></div> }
