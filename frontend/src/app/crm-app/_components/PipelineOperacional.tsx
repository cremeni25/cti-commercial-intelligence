...
const OPCOES_ETAPA = ETAPAS.map((item) => [item, item.replaceAll("_", " ")] as const)
...
function etapaEditavel(valor: string): string {
  ...
  if (["ACEITE", "DOSSIÊ", "DOSSIE", "CARRIER", "FATURADO"].includes(etapa)) return etapa
...
export default function PipelineOperacional() {
  ...
  async function carregar() {
    ...
    try {
      ...
      setErro(falha instanceof Error ? falha.message : "Não foi possível carregar o pipeline.")
    } finally {
      setCarregando(false)
    }
  }
  ...
  const etapasVisiveis = useMemo(() => {
    const base = [...ETAPAS]
    for (const negocio of visiveis) {
      if (!base.includes(negocio.etapa as typeof ETAPAS[number])) base.push(negocio.etapa as typeof ETAPAS[number])
    }
  ...
  const modoConcentrado = etapasVisiveis.length <= 2
  ...
  async function mover(negocio: Negocio) {
    ...
    try {
      ...
      setErro(falha instanceof Error ? falha.message : "Não foi possível mover a oportunidade.")
    } finally {
      setMovendo("")
    }
  }
  ...
  return (
    <main className="min-h-[100dvh] bg-[#020817] px-4 py-5 pb-28 text-white sm:px-6">
      <div className="mx-auto w-full max-w-[1500px]">
        <header className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <div className="flex min-w-0 items-start gap-3">
            <Link href="/crm-app" aria-label="Voltar ao CRM" className="grid size-11 shrink-0 place-items-center rounded-2xl border border-[#24466f] bg-[#07162b] text-cyan-300"><ArrowLeft size={19}/></Link>
            <div className="min-w-0"><p className="text-xs uppercase tracking-[.24em] text-cyan-400">CTI CRM</p><h1 className="text-2xl font-bold">Pipeline comercial</h1><p className="text-sm text-slate-400">Quadro operacional dos mesmos negócios do núcleo CTI</p></div>
          </div>
          <div className="flex gap-2"><button onClick={() => void carregar()} className="grid size-12 place-items-center rounded-2xl border border-[#24466f] text-cyan-300" aria-label="Atualizar"><RefreshCw size={18}/></button><Link href="/crm-app/forecast" className="flex items-center gap-2 rounded-2xl border border-cyan-700 px-4 py-3 text-sm font-semibold text-cyan-200"><TrendingUp size={17}/>Forecast</Link></div>
        </header>
        ...
        <section className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
          <Kpi label="Negócios" valor={String(visiveis.length)}/><Kpi label="Valor total" valor={moeda(total)}/><Kpi label="Ponderado" valor={moeda(ponderado)}/><Kpi label="Sem interação" valor={String(semInteracao)} alerta={semInteracao > 0}/><Kpi label="Fechamento vencido" valor={String(vencidos)} alerta={vencidos > 0}/>
        </section>
        ...
        {carregando ? (
          <div className="grid min-h-72 place-items-center"><Loader2 className="animate-spin text-cyan-300"/></div>
        ) : etapasVisiveis.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-[#24466f] p-10 text-center text-slate-400">Nenhum negócio no pipeline.</div>
        ) : (
          <div className={modoConcentrado ? "w-full" : "overflow-x-auto pb-4"}>
            <div className={modoConcentrado ? `grid w-full gap-4 ${etapasVisiveis.length === 2 ? "lg:grid-cols-2" : "grid-cols-1"}` : "flex min-w-max gap-4"}>
              {etapasVisiveis.map((etapa) => {
                const negocios = visiveis.filter((item) => item.etapa === etapa)
                const valorEtapa = negocios.reduce((s, item) => s + item.valor, 0)
                const gradeCards = modoConcentrado ? "grid gap-3 md:grid-cols-2 2xl:grid-cols-3" : "space-y-3"
                return (
                  <section key={etapa} className={modoConcentrado ? "min-w-0 rounded-3xl border border-[#16325c] bg-[#061126] p-4" : "w-[360px] shrink-0 rounded-3xl border border-[#16325c] bg-[#061126] p-3"}>
                    <header className="mb-4 rounded-2xl bg-[#0a2242] p-4"><div className="flex items-center justify-between gap-3"><strong className="text-base">{etapa.replaceAll("_", " ")}</strong><span className="rounded-full bg-cyan-950/70 px-2.5 py-1 text-xs text-cyan-200">{negocios.length}</span></div><p className="mt-1 text-xs text-slate-400">{moeda(valorEtapa)}</p></header>
                    <div className={gradeCards}>
                      {negocios.map((negocio) => {
                        const atrasado = Boolean(negocio.fechamento) && negocio.fechamento < hoje && !negocio.encerrada
                        return (
                          <article key={negocio.id} className="min-w-0 rounded-2xl border border-[#24466f] bg-[#07162b] p-4">
                            <h2 className="break-words font-bold leading-5">{negocio.cliente}</h2>
                            <p className="mt-1 break-words text-sm text-slate-300">{negocio.titulo}</p>
                            <div className="mt-3 space-y-1 text-xs text-slate-400"><p>{moeda(negocio.valor)} · {Math.round((negocio.probabilidade > 1 ? negocio.probabilidade : negocio.probabilidade * 100))}%</p><p className="inline-flex items-center gap-1"><CalendarDays size={13}/>{dataBR(negocio.fechamento)}</p>{negocio.atividades === 0 && !negocio.encerrada && <p className="flex items-center gap-1 text-amber-300"><CircleAlert size={13}/>Sem interação registrada</p>}{atrasado && <p className="flex items-center gap-1 text-red-300"><CircleAlert size={13}/>Fechamento previsto vencido</p>}</div>
                            <div className="mt-3 grid grid-cols-2 gap-2"><Link href={`/crm-app/historico/${negocio.id}?origem=pipeline`} className="flex items-center justify-center gap-1 rounded-xl border border-[#24466f] py-2 text-xs text-cyan-200"><History size={14}/>Histórico</Link><Link href={`/crm-app/atividades/nova?oportunidade=${negocio.id}&origem=pipeline`} className="flex items-center justify-center gap-1 rounded-xl border border-cyan-800 py-2 text-xs text-cyan-200"><MessageSquarePlus size={14}/>Interação</Link></div>
                            {!negocio.encerrada && <div className="mt-3 rounded-xl border border-[#24466f] p-2"><label className="mb-1 block text-[11px] text-slate-400">Mover para</label><ControlledSelect value={destinos[negocio.id] || negocio.etapa} onChange={(valor) => setDestinos((atual) => ({ ...atual, [negocio.id]: valor }))} options={OPCOES_ETAPA} buttonClassName="h-10 rounded-lg px-3 text-xs"/><button disabled={movendo === negocio.id || (destinos[negocio.id] || negocio.etapa) === negocio.etapa} onClick={() => void mover(negocio)} className="mt-2 flex h-9 w-full items-center justify-center gap-1 rounded-lg bg-cyan-500 text-xs font-bold text-slate-950 disabled:opacity-40">{movendo === negocio.id ? <Loader2 size={14} className="animate-spin"/> : <ArrowRight size={14}/>}Confirmar etapa</button></div>}
                          </article>
                        )
                      })}
                    </div>
...
function Kpi({ label, valor, alerta = false }: { label: string; valor: string; alerta?: boolean }) {
  return <div className={`min-w-0 overflow-hidden rounded-2xl border p-4 ${alerta ? "border-amber-800 bg-amber-950/20" : "border-[#16325c] bg-[#07162b]"}`}><p className="text-xs text-slate-400">{label}</p><strong className={`mt-1 block max-w-full break-words text-base leading-tight tabular-nums sm:text-lg ${alerta ? "text-amber-300" : "text-cyan-300"}`}>{valor}</strong></div>
}