...
export default function CrmModuloPage() {
  ...
  function alterarProbabilidade(valor: string) {
    ...
    setProbabilidade(Math.min(100, Number(digitos || 0)))
  }
  ...
  return <main className="min-h-[100dvh] bg-[#020817] pb-24 text-white">
    <header className="sticky top-0 z-30 border-b border-[#16325c] bg-[#061126]/95 px-4 py-3 backdrop-blur sm:px-6 sm:py-4">
      <div className="mx-auto flex w-full max-w-[94vw] items-center gap-3">
        ...
        <button onClick={() => void carregar()} className="grid size-11 place-items-center rounded-2xl border border-[#16325c] bg-[#091a33] text-cyan-300"><RefreshCw size={19} /></button>
      </div>
    </header>
    ...
    <div className="mx-auto w-full max-w-[94vw] px-4 py-4 sm:px-6 sm:py-6">
      ...
      {novo ? <form onSubmit={salvar} className="mt-4 grid gap-4 rounded-3xl border border-[#16325c] bg-[#07162b] p-4 sm:grid-cols-2 sm:p-6">
        ...
        {slug === "oportunidades" ? <>
          <label><span className="mb-2 block text-sm text-slate-300">Valor estimado</span><input inputMode="numeric" value={valorCentavos ? moeda.format(valorEstimado) : ""} onChange={(e) => alterarValorDigitado(e.target.value)} onFocus={(e) => e.currentTarget.select()} placeholder="R$ 0,00" className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4 outline-none focus:border-cyan-500" /></label>
          <label><span className="mb-2 block text-sm text-slate-300">Probabilidade</span><input inputMode="numeric" value={probabilidade ? `${probabilidade}%` : ""} onChange={(e) => alterarProbabilidade(e.target.value)} onFocus={(e) => e.currentTarget.select()} placeholder="0%" className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4 outline-none focus:border-cyan-500" /></label>
...
function SelecaoMultipla({ titulo, opcoes, selecionados, alterar }: { titulo: string; opcoes: string[]; selecionados: string[]; alterar: (valor: string[]) => void }) {
