...
@router.get("")
def listar_clientes_seguro(usuario: UsuarioAutenticado = Depends(usuario_atual)):
    """Catálogo cadastral global: pesquisável por qualquer usuário com permissão de visualizar clientes.

    A propriedade comercial não é aplicada aqui de propósito. Um vendedor pode localizar uma empresa
    existente para iniciar um novo negócio; a segregação acontece nas oportunidades/atividades/propostas.
    """
    _exigir_permissao(usuario, "clientes_visualizar")
    return listar_clientes_crm_app()
