...
const atalhos = [
  { href: "/crm-app/atividades/nova", titulo: "Nova atividade", descricao: "Registrar visita, ligação, reunião ou próxima ação", icon: ClipboardCheck },
...
export default function CrmAppPage() {
  ...
  const analises = [
    ...
    { href: "/crm-app/visitas", label: "Visitas", valor: resumo.visitas, descricao: "filtro das atividades de visita", icon: MapPinned },
    ...
    { href: "/crm-app/atividades", label: "Atividades", valor: resumo.atividades, descricao: "todas as interações registradas", icon: Activity },
  ]
  ...
  return <main className="min-h-[100dvh] bg-[#020817] pb-24 text-white">
    ...
    <div className="mx-auto w-full max-w-[94vw] px-4 py-4 sm:px-6 sm:py-6"><div className="grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(360px,0.65fr)]"><div className="space-y-5">
      <section className="rounded-3xl border border-[#16325c] bg-gradient-to-br from-[#0a2242] to-[#07162b] p-5 shadow-xl sm:p-7"><div className="flex items-start justify-between gap-4"><div><p className="text-sm text-slate-400 sm:text-base">Operação comercial diária</p><h2 className="mt-1 text-2xl font-bold sm:text-3xl">Olá, {usuario?.nome?.split(" ")[0] || "usuário CTI"}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300 sm:text-base">Registre todas as interações comerciais em uma única atividade. Visitas permanecem disponíveis como filtro de consulta.</p></div><button type="button" onClick={() => void sincronizar()} className="rounded-2xl border border-cyan-800 bg-cyan-950/30 p-3 text-cyan-300"><RefreshCw size={20} className={sincronizando ? "animate-spin" : ""} /></button></div><div className="mt-6 grid grid-cols-3 gap-3"><Indicador valor={resumo.visitas} label="Visitas hoje" /><Indicador valor={resumo.pendencias} label="Pendências" /><Indicador valor={resumo.oportunidades} label="Oportunidades" /></div></section>
