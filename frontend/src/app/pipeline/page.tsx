      ...
      <section className="min-w-0 flex-1">
        ...
        <div className="space-y-6 p-4 sm:p-6 lg:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="text-3xl font-bold sm:text-4xl">CRM • Pipeline Comercial</h1>
              <p className="mt-2 text-gray-400">Visão por estágio, com uma ficha compacta para cada negociação.</p>
            </div>
            <Link href="/oportunidades?novo=1" className="rounded-xl bg-cyan-500 px-5 py-3 text-center font-semibold text-slate-950">Nova oportunidade</Link>
          </div>

          <section className="grid gap-4 rounded-2xl border border-[#13203f] bg-[#071226] p-5 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_auto]">
          ...
          <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <Kpi titulo="Oportunidades no período" valor={quadro.resumo.total_oportunidades.toLocaleString("pt-BR")} />
          ...
          {loading ? <Aviso>Carregando pipeline...</Aviso> : quadro.cards.length === 0 ? <Aviso>Nenhuma oportunidade encontrada no período.</Aviso> : (
            <div className="overflow-x-auto pb-4">
              <div className="grid min-w-[1450px] grid-cols-5 gap-4">
                {etapas.map((etapa) => {
                  const cards = quadro.cards.filter((card) => card.etapa === etapa)
                  return (
                    <section key={etapa} className="min-h-[460px] rounded-2xl border border-[#13203f] bg-[#071028]">
...
function Card({ card }: { card: CardPipeline }) {
  return (
    <article className="rounded-xl border border-[#13203f] bg-[#091a33] p-4">
      <p className="text-sm font-semibold text-cyan-300">{card.cliente_nome}</p>
      <h3 className="mt-1 font-semibold text-white">{card.titulo}</h3>
      <div className="mt-3 space-y-1 text-xs text-gray-400">
        <p>{moeda(card.valor_estimado)} • {percentual(card.probabilidade)}</p>
        {card.equipamento && <p>{card.equipamento}</p>}
        {card.data_fechamento_prevista && <p>Previsão: {new Date(`${card.data_fechamento_prevista}T12:00:00`).toLocaleDateString("pt-BR")}</p>}
      </div>
      <Link href={`/oportunidades/${card.oportunidade_id}`} className="mt-4 inline-flex rounded-lg border border-cyan-700 px-3 py-2 text-xs font-semibold text-cyan-300">Ver detalhes</Link>
    </article>
  )
}
...
function Aviso({ children }: { children: React.ReactNode }) {
  return <div className="rounded-2xl border border-[#13203f] bg-[#091a33] p-10 text-gray-300">{children}</div>
}
