...
export default function HomologacaoPage() {
  ...
  async function carregar() {
    ...
    } catch (falha) { setErro(falha instanceof Error ? falha.message : "Não foi possível carregar a administração de homologação.") }
    finally { setCarregando(false) }
  }

  useEffect(() => { if (adminMaster) void carregar(); else if (usuario) setCarregando(false) }, [adminMaster, usuario])

  const totalAfetado = useMemo(() => previa ? Object.entries(previa.resumo).filter(([chave]) => chave !== "clientes_mestre_preservados").reduce((soma, [, valor]) => soma + Number(valor || 0), 0) : 0, [previa])
  ...
  async function arquivar() {
    ...
    try {
      ...
      await carregar()
    } catch (falha) { setErro(falha instanceof Error ? falha.message : "Falha ao arquivar histórico.") }
  ...
  async function restaurar(loteId: string) {
    ...
    try {
      ...
      await carregar()
    } catch (falha) { setErro(falha instanceof Error ? falha.message : "Falha ao restaurar lote.") }
    finally { setProcessando(false) }
  }
  ...
  return <main className="min-h-[100dvh] bg-[#020817] px-4 py-5 pb-24 text-white sm:px-6"><div className="mx-auto max-w-5xl">
    <header className="mb-5 flex items-center gap-3"><Link href="/crm-app/oportunidades" className="grid size-11 place-items-center rounded-2xl border border-[#16325c] bg-[#091a33] text-cyan-300"><ArrowLeft size={20}/></Link><div><p className="text-xs uppercase tracking-[.22em] text-amber-400">ADMIN_MASTER</p><h1 className="text-2xl font-bold">Administração de homologação</h1><p className="text-sm text-slate-400">Arquivamento coordenado e reversível do histórico de teste do CRM App.</p></div></header>
    ...
    <section className="rounded-3xl border border-amber-800/60 bg-amber-950/15 p-5 sm:p-6">
      <div className="flex items-start gap-3"><AlertTriangle className="mt-1 text-amber-300"/><div><h2 className="text-lg font-bold text-amber-100">Prévia do lote ativo</h2><p className="mt-1 text-sm leading-6 text-amber-100/70">Nada é apagado. Os registros deixam as fontes operacionais, mas permanecem preservados e auditáveis para eventual restauração.</p></div></div>
      {previa && <><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-5">{Object.entries(previa.resumo).map(([chave, valor]) => <div key={chave} className="rounded-2xl border border-[#24466f] bg-[#07162b] p-3"><strong className="block text-xl text-cyan-300">{valor}</strong><span className="text-xs text-slate-400">{chave.replaceAll("_", " ")}</span></div>)}</div>
      <p className="mt-4 rounded-xl border border-cyan-900 bg-cyan-950/20 p-3 text-xs leading-5 text-cyan-100">{previa.aviso_clientes}</p>
      <div className="mt-4 space-y-2">{previa.oportunidades.map(item => <div key={String(item.id)} className="rounded-xl border border-[#16325c] bg-[#061126] px-4 py-3 text-sm"><strong>{String(item.cliente_nome || "Cliente")}</strong><span className="text-slate-400"> · {String(item.titulo || "Oportunidade")} · {String(item.status || "")}</span></div>)}</div>
      <div className="mt-5 rounded-2xl border border-red-900/70 bg-red-950/20 p-4"><p className="text-sm font-semibold text-red-200">Confirmação obrigatória</p><p className="mt-1 text-xs text-red-100/70">Esta ação arquivará {totalAfetado} registros transacionais conectados à prévia acima.</p><input value={confirmacao} onChange={e => setConfirmacao(e.target.value)} placeholder={previa.confirmacao_exigida} className="mt-3 h-12 w-full rounded-xl border border-red-800 bg-[#020817] px-4 text-sm"/><button type="button" onClick={()=>void arquivar()} disabled={processando || previa.oportunidade_ids.length===0 || confirmacao.trim().toUpperCase()!==previa.confirmacao_exigida} className="mt-3 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-red-500 font-bold text-white disabled:opacity-40">{processando?<Loader2 className="animate-spin" size={18}/>:<Archive size={18}/>}Arquivar histórico de homologação</button></div></>}
    </section>
