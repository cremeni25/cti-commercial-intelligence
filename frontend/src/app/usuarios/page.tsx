...
import { criarUsuario, listarUsuarios, type PermissoesUsuario, type UsuarioNovo } from "@/modules/usuarios/services/usuarios.service"
...
export default function Page() {
  ...
  function marcar(chave: keyof PermissoesUsuario, valor: boolean) {
          ...
          {authLoading || loading ? <Aviso>Carregando usuários...</Aviso> : !autorizado ? <Aviso>Este módulo é exclusivo do Admin Master.</Aviso> : <>
            ...
            {mostrarNovo && <form onSubmit={cadastrar} className="space-y-6 rounded-3xl border border-cyan-900 bg-[#071427] p-5 sm:p-6">
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                ...
                <Campo label="Senha temporária" type="password" value={novo.senha_temporaria} onChange={(value) => setNovo({ ...novo, senha_temporaria: value })} required />
                <Campo label="Cargo ou função" value={novo.funcao} onChange={(value) => setNovo({ ...novo, funcao: value })} placeholder="Ex.: Diretor Comercial, Vendedor, Financeiro" required />
