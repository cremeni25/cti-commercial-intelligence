      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap gap-2">
            {contextos.map(([value, label]) => (
              <button key={value} onClick={() => setContexto(value)} className={`rounded-lg px-4 py-2 text-sm font-medium ${contexto === value ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-700"}`}>{label}</button>
            ))}
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            {segmentos.map((value) => (
              <button key={value} onClick={() => setSegmento(value)} className={`rounded-lg px-4 py-2 text-sm font-medium ${segmento === value ? "bg-blue-700 text-white" : "bg-blue-50 text-blue-800"}`}>{value}</button>
            ))}
          </div>
        </section>
        ...
        {erro && (
          <div className="rounded-xl border border-red-200 bg-red-50 p-6">
            ...
            <button onClick={() => void carregar()} className="mt-4 rounded-lg bg-red-700 px-4 py-2 text-sm font-medium text-white">Recarregar</button>
          </div>
        )}
