...
type Payload = {
  metadata:{nome_contrato:string;competencia:string;natureza:string;market_share_oficial:boolean;escopo_usuario?:{usuario?:string}}
...
export default function AnfirWorkbookPanel({responsavelId}:{responsavelId?:string}){
  ...
  return <section className="rounded-3xl border border-cyan-500/30 bg-[#061126] p-5 sm:p-6">
    ...
    {tab==="viena"&&<div className="mt-6 space-y-6">
      ...
      <Table headers={["Segmento","Mercado","Carrier","Carrier %","TK","Nacional","Sem contato","Não participou","Q2 vs Q1"]} rows={v.segmentos.map(s=>{const familia=FAMILY_SLUG[s.codigo];return [cellLink(s.segmento,drill({titulo:`${s.segmento} · mercado ANFIR 2026`,familia})),cellLink(formatNumber(s.mercado),drill({titulo:`${s.segmento} · mercado ANFIR 2026`,familia})),cellLink(formatNumber(s.carrier),drill({titulo:`${s.segmento} · Carrier observada`,familia,campo:"categoria",valor:"CARRIER"})),`${s.carrier_percentual_observado.toFixed(1)}%`,cellLink(formatNumber(s.tk),drill({titulo:`${s.segmento} · TK`,familia,campo:"categoria",valor:"TK"})),cellLink(formatNumber(s.nacional),drill({titulo:`${s.segmento} · Nacional`,familia,campo:"categoria",valor:"NACIONAL"})),cellLink(formatNumber(s.sem_contato),drill({titulo:`${s.segmento} · Sem contato`,familia,campo:"categoria",valor:"SEM_CONTATO"})),cellLink(formatNumber(s.nao_participou),drill({titulo:`${s.segmento} · Não participamos da proposta`,familia,campo:"causa",valor:"COBERTURA_COMERCIAL"})),s.q2_vs_q1_percentual===null?"—":`${s.q2_vs_q1_percentual>0?"+":""}${s.q2_vs_q1_percentual.toFixed(1)}%`]})}/>

      <div className="grid gap-5 xl:grid-cols-2">
        <Card title="Evolução mensal 2026"><Table compact headers={["Mês","Trailer","Diesel Truck","Direct Drive","Total"]} rows={v.mensal.map(m=>[cellLink(m.mes,drill({titulo:`${m.mes} 2026 · ANFIR`,campo:"mes",valor:m.competencia})),cellLink(formatNumber(m.trailer),drill({titulo:`${m.mes} 2026 · Trailer`,campo:"mes",valor:m.competencia,familia:"trailer"})),cellLink(formatNumber(m.diesel_truck),drill({titulo:`${m.mes} 2026 · Diesel Truck`,campo:"mes",valor:m.competencia,familia:"diesel-truck"})),cellLink(formatNumber(m.direct_drive),drill({titulo:`${m.mes} 2026 · Direct Drive`,campo:"mes",valor:m.competencia,familia:"direct-drive"})),cellLink(formatNumber(m.total),drill({titulo:`${m.mes} 2026 · total ANFIR`,campo:"mes",valor:m.competencia}))])}/></Card>
        <Card title="Território por DDD"><Table compact headers={["DDD","Mercado","Trailer","Diesel Truck","Direct Drive","Responsável"]} rows={v.territorio.map(d=>[cellLink(d.ddd,drill({titulo:`DDD ${d.ddd} · ANFIR 2026`,campo:"ddd",valor:d.ddd})),cellLink(formatNumber(d.mercado),drill({titulo:`DDD ${d.ddd} · mercado ANFIR`,campo:"ddd",valor:d.ddd})),cellLink(formatNumber(d.trailer),drill({titulo:`DDD ${d.ddd} · Trailer`,campo:"ddd",valor:d.ddd,familia:"trailer"})),cellLink(formatNumber(d.diesel_truck),drill({titulo:`DDD ${d.ddd} · Diesel Truck`,campo:"ddd",valor:d.ddd,familia:"diesel-truck"})),cellLink(formatNumber(d.direct_drive),drill({titulo:`DDD ${d.ddd} · Direct Drive`,campo:"ddd",valor:d.ddd,familia:"direct-drive"})),d.responsavel])}/></Card>
      </div>
