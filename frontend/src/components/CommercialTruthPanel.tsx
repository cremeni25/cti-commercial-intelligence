...
type Resumo={evidencias:number;clientes_reconciliados:number;evidencias_sem_cliente_reconciliado:number;por_fonte:Record<string,number>;por_desfecho:Record<string,number>;cadeias_crm_funil_realizado:number;cadeias_temporais_confirmadas:number}
type Jornada={cliente_id:string;cliente_nome:string;cidade?:string|null;ddd?:string|null;origens:string[];quantidade_evidencias:number;primeiro_evento?:string|null;ultimo_evento?:string|null;desfecho:string;cadeia_crm_funil_realizado:boolean;ordem_temporal_confirmada:boolean}
...
export default function CommercialTruthPanel({responsavelId}:{responsavelId?:string}){
 ...
 useEffect(()=>{let ativo=true;const q=new URLSearchParams({limite_clientes:"20"});if(responsavelId)q.set("responsavel_id",responsavelId);setLoading(true);void buscarSeguro(`/api/cti/analytics/verdade-comercial?${q.toString()}`).then(p=>{if(ativo){setData(p);setErro("")}}).catch(e=>{if(ativo)setErro(e instanceof Error?e.message:"Falha ao carregar reconciliação comercial.")}).finally(()=>{if(ativo)setLoading(false)});return()=>{ativo=false}},[responsavelId])
 ...
 return <section className="rounded-3xl border border-emerald-500/25 bg-[#071427] p-5 sm:p-6">
  <div><p className="text-xs font-semibold uppercase tracking-[.18em] text-emerald-300">Consistência transversal</p><h2 className="mt-2 text-2xl font-bold">Jornada comercial reconciliada</h2><p className="mt-2 max-w-5xl text-sm leading-6 text-slate-400">ANFIR continua sendo passado confirmado; Funil representa encerrados e negócios em backlog; CRM representa a ação operacional diária. O CTI correlaciona as fontes pelo mesmo cliente sem alterar o dado original.</p></div>
  <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Kpi label="Clientes reconciliados" value={r.clientes_reconciliados}/><Kpi label="CRM → Funil → realizado" value={r.cadeias_crm_funil_realizado}/><Kpi label="Ordem temporal confirmada" value={r.cadeias_temporais_confirmadas}/><Kpi label="Sucessos confirmados" value={r.por_desfecho.SUCESSO_COMERCIAL_CONFIRMADO||0}/><Kpi label="Em curso / backlog" value={r.por_desfecho.EM_CURSO_BACKLOG||0}/></div>
  <div className="mt-5 rounded-2xl border border-[#17304d] bg-[#061126] p-4"><p className="text-sm font-semibold">Regra oficial de leitura</p><div className="mt-3 grid gap-3 text-xs text-slate-300 md:grid-cols-3"><p><strong className="text-cyan-300">ANFIR:</strong> fato de mercado já encerrado.</p><p><strong className="text-amber-300">Funil:</strong> realizado histórico + backlog/em andamento.</p><p><strong className="text-emerald-300">CRM:</strong> visita, agenda, prospecção e ação diária.</p></div></div>
  {top.length>0&&<div className="mt-5 overflow-x-auto rounded-2xl border border-[#17304d]"><table className="min-w-full text-sm"><thead className="bg-[#0b1b34] text-left text-xs uppercase text-slate-400"><tr><th className="p-3">Cliente</th><th className="p-3">Fontes encontradas</th><th className="p-3">Desfecho</th><th className="p-3">Evidências</th><th className="p-3">Cadeia temporal</th></tr></thead><tbody>{top.map(j=><tr key={j.cliente_id} className="border-t border-[#17304d]"><td className="p-3"><p className="font-semibold text-cyan-200">{j.cliente_nome}</p><p className="text-xs text-slate-500">{[j.cidade,j.ddd].filter(Boolean).join(" · ")}</p></td><td className="p-3">{j.origens.join(" → ")}</td><td className="p-3 font-semibold">{rotuloDesfecho(j.desfecho)}</td><td className="p-3">{j.quantidade_evidencias}</td><td className="p-3">{j.ordem_temporal_confirmada?"Confirmada":"Ainda não confirmada"}</td></tr>)}</tbody></table></div>}
