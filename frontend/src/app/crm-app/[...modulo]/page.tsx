...
import {
  Activity, ArrowLeft, BriefcaseBusiness, Building2, CalendarDays,
  CheckCircle2, CircleDollarSign, Clock3, Loader2, MapPinned,
  Plus, Search, Target, UserPlus,
} from "lucide-react"
import { useAuth } from "@/core/auth"
import { lerContextoOportunidade, montarDescricaoComContexto, textoSeguro } from "@/lib/crm-opportunity"
...
const configs: Record<string, ModuloConfig> = {
  ...
  atividades: { titulo: "Interações", endpoint: "/crm/atividades", icon: Activity, extrair: (p) => Array.isArray(p) ? p : [] },
}
...
function clienteDe(item: Registro): ClienteOpcao | null {
  const nome = textoSeguro(item.razao_social || item.nome || item.nome_fantasia || item.empresa || item.cliente).trim()
  if (!nome) return null
  return {
    id: textoSeguro(item.id || item.cliente_id || item.uuid).trim() || nome,
    nome,
    codigo: textoSeguro(item.codigo || item.codigo_cliente || item.id).trim() || codigoEstavel(nome),
...
export default function CrmModuloPage() {
  ...
  const [novoCliente, setNovoCliente] = useState({ nome: "", segmento: "TRANSPORTADOR" })
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      setErro(falha instanceof Error ? falha.message : "Não foi possível carregar os dados deste módulo.")
    } finally {
      setCarregando(false)
    }
  }, [api, config])
  ...
  async function salvar(evento: FormEvent<HTMLFormElement>) {
    ...
    if (slug === "oportunidades") {
      ...
      if (!nomeCliente || !municipio.trim() || uf.trim().length !== 2 || !equipamentos.length || quantidade < 1) {
      ...
      const descricao = montarDescricaoComContexto(String(dados.get("descricao") || "").trim(), {
        linhas, equipamentos, quantidade, municipio: municipio.trim(), uf: uf.trim().toUpperCase(), ddd: ddd.trim(), subRegiao: subRegiao.trim(),
      })
      const payload = {
        cliente: {
      ...
      try {
        ...
        setLinhas([]); setEquipamentos([]); setQuantidade(1); setValorCentavos(0); setProbabilidade(0)
        setSucesso("Oportunidade registrada no CTI e disponível no site e no App.")
        await carregar()
      } catch (falha) {
        setErro(falha instanceof Error ? falha.message : "Não foi possível salvar a oportunidade.")
    ...
    const payload = {
      ...
      usuario_id: userId,
      tipo: String(dados.get("tipo") || "FOLLOW_UP"),
    ...
    try {
      ...
      const detalhe = await resposta.json().catch(() => ({}))
      if (!resposta.ok) throw new Error(String(detalhe.detail || `Falha ${resposta.status}`))
      formulario.reset(); setClienteBusca(""); setClienteSelecionado(null)
      setSucesso("Interação registrada com sucesso.")
      await carregar()
    } catch (falha) {
      setErro(falha instanceof Error ? falha.message : "Não foi possível salvar a interação.")
    } finally { setSalvando(false) }
  }

  async function concluir(id: string) {
    const resposta = await api(`/crm/atividades/${id}/concluir`, { method: "PUT" })
    if (!resposta.ok) setErro("Não foi possível concluir a interação.")
    else await carregar()
  }

  async function atualizarOportunidade(id: string, payload: Registro) {
    const resposta = await api(`/crm/oportunidades/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })
    if (!resposta.ok) {
      const detalhe = await resposta.json().catch(() => ({}))
      throw new Error(String(detalhe.detail || "Não foi possível atualizar a oportunidade."))
    }
    await carregar()
  }
    ...
    <main className="min-h-[100dvh] bg-[#020817] pb-24 text-white">
      <header className="sticky top-0 z-30 border-b border-[#16325c] bg-[#061126]/95 px-4 py-3 backdrop-blur sm:px-6 sm:py-4">
        <div className="mx-auto flex w-full max-w-[94vw] items-center gap-3">
          ...
          <div className="min-w-0 flex-1"><p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-cyan-400">CTI CRM</p><h1 className="truncate text-lg font-bold sm:text-2xl">{novo ? `Novo registro — ${config.titulo}` : config.titulo}</h1></div>
        </div>
      </header>
      ...
      <div className="mx-auto w-full max-w-[94vw] px-4 py-4 sm:px-6 sm:py-6">
        <section className="rounded-3xl border border-[#16325c] bg-gradient-to-br from-[#0a2242] to-[#07162b] p-4 shadow-xl sm:p-6">
          <div className="flex items-center gap-3"><span className="grid size-12 place-items-center rounded-2xl bg-cyan-950/60 text-cyan-300"><Icon size={24} /></span><div><p className="text-sm text-slate-400">Operação em campo</p><p className="text-xl font-bold">{config.titulo}</p></div></div>
        </section>
        ...
        {novo ? (
          <form onSubmit={salvar} className="mt-4 grid gap-4 rounded-3xl border border-[#16325c] bg-[#07162b] p-4 sm:grid-cols-2 sm:p-6">
            <label className="relative">
              ...
              <input value={clienteBusca} onChange={(e) => { setClienteBusca(e.target.value); setClienteSelecionado(null); setNovoCliente((a) => ({ ...a, nome: e.target.value })) }} placeholder="Digite ao menos 2 letras" autoComplete="off" className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4" />
              {sugestoesClientes.length > 0 && <div className="absolute z-40 mt-2 max-h-72 w-full overflow-auto rounded-2xl border border-[#24466f] bg-[#07162b] shadow-2xl">{sugestoesClientes.map((c) => <button type="button" key={`${c.id}-${c.codigo}`} onClick={() => selecionarCliente(c)} className="block w-full border-b border-[#16325c] px-4 py-3 text-left last:border-0"><strong className="block">{c.nome}</strong><span className="text-xs text-slate-400">Código CTI: {c.codigo}</span></button>)}</div>}
              {clienteSelecionado && <span className="mt-2 block text-xs text-emerald-300">Cliente selecionado • código {clienteSelecionado.codigo}</span>}
              {clienteBusca.trim().length >= 2 && !clienteSelecionado && sugestoesClientes.length === 0 && <button type="button" onClick={() => setCadastroNovo(true)} className="mt-2 inline-flex items-center gap-2 text-sm font-semibold text-cyan-300"><UserPlus size={16} /> Cadastrar “{clienteBusca.trim()}” como novo cliente</button>}
            </label>
            <Campo name="titulo" label={slug === "oportunidades" ? "Título da oportunidade" : "Objetivo da interação"} required />

            {cadastroNovo && <fieldset className="grid gap-3 rounded-2xl border border-cyan-800 bg-cyan-950/20 p-4 sm:col-span-2 sm:grid-cols-3"><legend className="px-2 font-semibold text-cyan-300">Cadastro territorial do novo cliente</legend><input value={novoCliente.nome} onChange={(e) => setNovoCliente({ ...novoCliente, nome: e.target.value })} placeholder="Razão social / nome" className="h-12 rounded-2xl border border-[#24466f] bg-[#020817] px-4 sm:col-span-2" /><select value={novoCliente.segmento} onChange={(e) => setNovoCliente({ ...novoCliente, segmento: e.target.value })} className="h-12 rounded-2xl border border-[#24466f] bg-[#020817] px-4"><option value="TRANSPORTADOR">Transportador</option><option value="LOCADORA">Locadora</option><option value="IMPLEMENTADORA">Implementadora</option><option value="DISTRIBUIDOR">Distribuidor</option><option value="INDUSTRIA">Indústria</option><option value="OUTRO">Outro</option></select></fieldset>}

            {slug === "oportunidades" ? <>
              <label><span className="mb-2 block text-sm text-slate-300">Valor estimado total</span><input inputMode="numeric" value={valorCentavos ? moeda.format(valorEstimado) : ""} onChange={(e) => alterarValorDigitado(e.target.value)} onFocus={(e) => e.currentTarget.select()} placeholder="R$ 0,00" className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4" /></label>
              <label><span className="mb-2 block text-sm text-slate-300">Chance estimada de fechamento</span><input inputMode="numeric" value={probabilidade ? `${probabilidade}%` : ""} onChange={(e) => setProbabilidade(Math.min(100, Number(e.target.value.replace(/\D/g, "") || 0)))} placeholder="0%" className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4" /></label>
              ...
              <CampoControlado label="Quantidade total de equipamentos" value={String(quantidade)} onChange={(v) => setQuantidade(Math.max(1, Number(v.replace(/\D/g, "") || 1)))} />
              ...
              <label><span className="mb-2 block text-sm text-slate-300">Tipo da interação</span><select name="tipo" defaultValue="FOLLOW_UP" className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4"><option value="VISITA">Visita</option><option value="LIGACAO">Ligação</option><option value="REUNIAO">Reunião</option><option value="FOLLOW_UP">Follow-up</option><option value="CONFIRMACAO">Confirmação</option><option value="ENVIO_PEDIDO">Envio de pedido</option><option value="RETORNO">Retorno</option><option value="OUTRA">Outra</option></select></label>
              <Campo name="data" label="Data" type="date" /><Campo name="horario" label="Horário" type="time" />
            </>}
            ...
            <button disabled={salvando} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-cyan-500 px-5 font-bold text-slate-950 disabled:opacity-60 sm:col-span-2">{salvando ? <Loader2 className="animate-spin" size={20} /> : <CheckCircle2 size={20} />} Salvar no CTI</button>
          </form>
        ) : (
          <section className="mt-4">
            <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} /><input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar neste módulo" className="h-12 w-full rounded-2xl border border-[#16325c] bg-[#07162b] pl-11 pr-4" /></div>
            {carregando ? <div className="grid min-h-48 place-items-center"><Loader2 className="animate-spin text-cyan-300" /></div> : filtrados.length === 0 ? <div className="mt-4 rounded-3xl border border-dashed border-[#24466f] p-8 text-center text-slate-400">Nenhum registro encontrado.</div> : <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{filtrados.map((item, indice) => <Card key={String(item.id || indice)} slug={slug} item={item} concluir={concluir} atualizar={atualizarOportunidade} />)}</div>}
          </section>
        )}
      </div>
...
function Campo({ name, label, type = "text", required = false, defaultValue }: { name: string; label: string; type?: string; required?: boolean; defaultValue?: string }) {
  return <label><span className="mb-2 block text-sm text-slate-300">{label}</span><input name={name} type={type} required={required} defaultValue={defaultValue} className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4" /></label>
}
function CampoControlado({ label, value, onChange, placeholder = "" }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return <label><span className="mb-2 block text-sm text-slate-300">{label}</span><input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4" /></label>
}
function SelecaoMultipla({ titulo, opcoes, selecionados, alterar }: { titulo: string; opcoes: string[]; selecionados: string[]; alterar: (v: string[]) => void }) {
  return <fieldset className="rounded-2xl border border-[#24466f] p-4"><legend className="px-2 text-sm text-slate-300">{titulo}</legend><div className="grid gap-2 sm:grid-cols-2">{opcoes.length === 0 ? <span className="text-sm text-slate-500">Selecione primeiro uma linha.</span> : opcoes.map((opcao) => <label key={opcao} className="flex items-center gap-2 rounded-xl bg-[#020817] px-3 py-2"><input type="checkbox" checked={selecionados.includes(opcao)} onChange={(e) => alterar(e.target.checked ? [...selecionados, opcao] : selecionados.filter((i) => i !== opcao))} /><span>{opcao}</span></label>)}</div></fieldset>
}

function Card({ slug, item, concluir, atualizar }: { slug: string; item: Registro; concluir: (id: string) => void; atualizar: (id: string, payload: Registro) => Promise<void> }) {
  ...
  const [data, setData] = useState(String(item.data_fechamento_prevista || "").slice(0, 10))
  const titulo = textoSeguro(item.titulo || item.nome || item.razao_social || item.oportunidade_titulo) || "Registro CTI"
  const subtitulo = textoSeguro(item.cliente_nome || item.cliente_id || item.municipio || item.empresa || item.tipo || item.etapa)
  const estado = textoSeguro(item.situacao || item.status || item.etapa) || "ATIVO"
  const dataRegistro = textoSeguro(item.data || item.data_fechamento_prevista || item.ultima_movimentacao)
  ...
  async function salvarEdicao() {
    ...
    try {
      const descricao = montarDescricaoComContexto(contexto.descricao, { linhas: contexto.linhas, equipamentos: contexto.equipamentos, quantidade, municipio: contexto.municipio, uf: contexto.uf, ddd: contexto.ddd, subRegiao: contexto.subRegiao })
      await atualizar(String(item.id), { valor_estimado: valor, probabilidade: Math.max(0, Math.min(100, chance)) / 100, status, data_fechamento_prevista: data || null, descricao })
      setEditando(false)
    } catch (falha) { setErroEdicao(falha instanceof Error ? falha.message : "Não foi possível atualizar a oportunidade.") }
    finally { setSalvando(false) }
  }

  return <article className="rounded-3xl border border-[#16325c] bg-[#07162b] p-4 shadow-lg sm:p-5">
    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="font-semibold sm:text-lg">{titulo}</h2><p className="mt-1 break-all text-sm text-slate-400">{subtitulo}</p></div><span className="rounded-full border border-cyan-900 bg-cyan-950/40 px-2.5 py-1 text-[10px] font-semibold text-cyan-300">{estado}</span></div>
    {oportunidade && !editando && <div className="mt-4 space-y-2 text-sm"><p><span className="text-slate-400">Produto:</span> {contexto.equipamentos.join(", ") || "A definir"}</p><p><span className="text-slate-400">Quantidade:</span> {contexto.quantidade} unidade(s)</p><p><span className="text-slate-400">Valor total:</span> <strong className="text-emerald-300">{moeda.format(Number(item.valor_estimado || 0))}</strong></p><p><span className="text-slate-400">Chance estimada:</span> {percentualExibido(item.probabilidade)}%</p>{contexto.descricao && <p className="text-slate-300">{contexto.descricao}</p>}</div>}
    {oportunidade && editando && <div className="mt-4 grid gap-3"><input type="number" min="0" step="0.01" value={valor} onChange={(e) => setValor(Number(e.target.value))} className="h-11 rounded-xl border border-[#24466f] bg-[#020817] px-3" placeholder="Valor total" /><input type="number" min="0" max="100" value={chance} onChange={(e) => setChance(Number(e.target.value))} className="h-11 rounded-xl border border-[#24466f] bg-[#020817] px-3" placeholder="Chance" /><input type="number" min="1" value={quantidade} onChange={(e) => setQuantidade(Math.max(1, Number(e.target.value)))} className="h-11 rounded-xl border border-[#24466f] bg-[#020817] px-3" placeholder="Quantidade" /><select value={status} onChange={(e) => setStatus(e.target.value)} className="h-11 rounded-xl border border-[#24466f] bg-[#020817] px-3"><option value="OPORTUNIDADE">OPORTUNIDADE</option><option value="PROPOSTA">PROPOSTA</option><option value="NEGOCIACAO">NEGOCIAÇÃO</option><option value="PEDIDO">PEDIDO</option><option value="DOSSIÊ">DOSSIÊ</option><option value="CARRIER">CARRIER</option><option value="GANHO">GANHO</option><option value="PERDIDO">PERDIDO</option><option value="CANCELADO">CANCELADO</option></select><input type="date" value={data} onChange={(e) => setData(e.target.value)} className="h-11 rounded-xl border border-[#24466f] bg-[#020817] px-3" />{erroEdicao && <p className="text-sm text-red-300">{erroEdicao}</p>}<div className="flex gap-2"><button type="button" disabled={salvando} onClick={() => void salvarEdicao()} className="flex min-h-10 flex-1 items-center justify-center gap-2 rounded-xl bg-cyan-500 font-semibold text-slate-950 disabled:opacity-60">{salvando ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={16} />} Salvar</button><button type="button" onClick={() => { setEditando(false); setErroEdicao("") }} className="min-h-10 rounded-xl border border-[#24466f] px-4">Cancelar</button></div></div>}
    {dataRegistro && <p className="mt-4 flex items-center gap-2 text-xs text-slate-400"><Clock3 size={14} /> {dataRegistro.slice(0, 16).replace("T", " ")}</p>}
    {oportunidade && !editando && Boolean(item.id) && <button type="button" onClick={() => setEditando(true)} className="mt-4 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border border-cyan-800 bg-cyan-950/30 text-sm text-cyan-300"><CheckCircle2 size={16} /> Alterar oportunidade</button>}
    {["agenda", "atividades", "visitas"].includes(slug) && estado !== "CONCLUIDA" && Boolean(item.id) ? <button type="button" onClick={() => concluir(String(item.id))} className="mt-4 inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border border-emerald-800 bg-emerald-950/30 text-sm text-emerald-300"><CheckCircle2 size={16} /> Concluir</button> : null}
  </article>
}
