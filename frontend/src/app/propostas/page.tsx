...
const textos = {
  "pt-BR": {
    subtitle:"Documento comercial da negociação: versão, emissão e aceite. Após a conversão, o registro segue para Pedidos.", dealsWithProposal:"Negociações com proposta", consolidatedValue:"Valor comercial consolidado", acceptedConverted:"Aceitas", status:"Status", all:"Todos", title:"Propostas comerciais", explanation:"Aqui se administra o documento até o aceite; após a conversão, o pedido aparece na etapa Pedidos.", currentFilter:"Filtro atual", loading:"Carregando...", client:"Cliente", currentProposal:"Proposta vigente", opportunity:"Oportunidade", value:"Valor", validity:"Validade", versions:"Versões", action:"Ação", noNumber:"Proposta sem número", data:"Dados", view:"Visualizar proposta", unidentifiedClient:"Cliente não identificado", genericOpportunity:"Oportunidade comercial", notProvided:"Não informada", click:"Clique para detalhar",
  ...
  en: {
    subtitle:"Commercial document for the deal: versioning, issuance and acceptance. After conversion, the record moves to Orders.", dealsWithProposal:"Deals with proposals", consolidatedValue:"Consolidated proposal value", acceptedConverted:"Accepted", status:"Status", all:"All", title:"Sales proposals", explanation:"This view manages the document through acceptance; after conversion, the order appears in the Orders stage.", currentFilter:"Current filter", loading:"Loading...", client:"Account", currentProposal:"Current proposal", opportunity:"Opportunity", value:"Value", validity:"Valid until", versions:"Versions", action:"Action", noNumber:"Proposal without number", data:"Details", view:"View proposal", unidentifiedClient:"Account not identified", genericOpportunity:"Commercial opportunity", notProvided:"Not provided", click:"Click to view details",
  ...
  es: {
    subtitle:"Documento comercial de la negociación: versión, emisión y aceptación. Después de la conversión, el registro pasa a Pedidos.", dealsWithProposal:"Negocios con propuesta", consolidatedValue:"Valor comercial consolidado", acceptedConverted:"Aceptadas", status:"Estado", all:"Todos", title:"Propuestas comerciales", explanation:"Aquí se administra el documento hasta la aceptación; después de la conversión, el pedido aparece en la etapa Pedidos.", currentFilter:"Filtro actual", loading:"Cargando...", client:"Cliente", currentProposal:"Propuesta vigente", opportunity:"Oportunidad", value:"Valor", validity:"Vigencia", versions:"Versiones", action:"Acción", noNumber:"Propuesta sin número", data:"Datos", view:"Ver propuesta", unidentifiedClient:"Cliente no identificado", genericOpportunity:"Oportunidad comercial", notProvided:"No informado", click:"Haga clic para ver detalles",
...
export default function PropostasPage() {
  ...
  const gruposOperacionais=grupos.filter(g=>statusCodigo(g.proposta)!=="CONVERTIDA_PEDIDO")
  const filtrados=gruposOperacionais.filter(g=>correspondeFiltro(g.proposta,filtro)),valor=gruposOperacionais.reduce((s,g)=>s+Number(g.proposta.valor||0),0),aceitas=gruposOperacionais.filter(g=>aceita(g.proposta)).length
  ...
  return <main className="flex min-h-screen bg-[#020817]"><Sidebar/><section className="flex-1"><Topbar/><div className="space-y-6 p-8">
    ...
    <section className="grid gap-4 md:grid-cols-3"><Kpi titulo={tx.dealsWithProposal} valor={formatNumber(gruposOperacionais.length)} onOpen={()=>abrirComposicao("TODOS")} detalhe={tx.click}/><Kpi titulo={tx.consolidatedValue} valor={formatCurrency(valor)} onOpen={()=>abrirComposicao("TODOS")} detalhe={tx.click}/><Kpi titulo={tx.acceptedConverted} valor={formatNumber(aceitas)} onOpen={()=>abrirComposicao("ACEITAS")} detalhe={tx.click}/></section>
