...
export function exportarRelatorioCrmPdf({
  ...
  if (!janela) {
    window.alert("O navegador bloqueou a abertura do relatório. Permita pop-ups para o CTI e tente novamente.")
    return
  }
  ...
  </style></head><body><h1>${escapar(titulo)}</h1>${subtitulo ? `<p>${escapar(subtitulo)}</p>` : ""}<div class="meta">Gerado em ${escapar(geradoEm)} · ${registros.length} registro(s)</div><table><thead><tr>${cabecalho}</tr></thead><tbody>${linhas}</tbody></table><div class="rodape">CTI Inteligência Comercial · relatório baseado na seleção exibida no CRM.</div><script>
