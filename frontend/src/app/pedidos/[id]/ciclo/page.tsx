...
type Ciclo = {
  ...
  status_ciclo:string
  etapas:string[]
  envio_carrier_confirmado?:boolean
  ...
  modalidade_venda?:"DIRETA"|"INDIRETA"
  marco_venda?:"NF_CONFIRMADA"|"ENCERRAMENTO_POS_VENDA"
...
export default function CicloPedidoPage(){
 ...
 return <main className="flex min-h-screen bg-[#020817] text-white"><Sidebar/><section className="min-w-0 flex-1"><Topbar/><div className="space-y-6 p-4 sm:p-6 lg:p-8">
  ...
  {dados&&<>
   ...
   <section className="grid gap-5 lg:grid-cols-2"><article className="rounded-3xl border border-[#13203f] bg-[#071427] p-6"><h2 className="text-xl font-bold">Comprovações e rastreabilidade</h2><dl className="mt-5 space-y-3 text-sm"><Linha label="CARRIER" valor={dataHora(dados.carrier_confirmado_em)}/><Linha label="Faturamento / marco da venda direta" valor={dataHora(dados.faturado_em)}/><Linha label="NF" valor={dados.numero_nf||"-"}/><Linha label="Número de série da NF" valor={dados.numero_serie_nf||"-"}/><Linha label="Entrega" valor={dataHora(dados.entregue_em)}/><Linha label="Instalação / acompanhamento ANFIR" valor={dataHora(dados.instalado_em)}/><Linha label="Número de série instalado" valor={dados.numero_serie_instalado||"-"}/><Linha label="Encerramento" valor={dataHora(dados.encerrado_em)}/></dl></article>
   <article className="rounded-3xl border border-[#13203f] bg-[#071427] p-6"><h2 className="text-xl font-bold">Atualizar acompanhamento</h2><p className="mt-2 text-sm text-slate-400">Etapa atual: <strong className="text-cyan-300">{atual}</strong></p>{proxima?.id==="CARRIER"?<div className="mt-5 rounded-2xl border border-amber-700 bg-amber-950/20 p-4 text-sm text-amber-200"><strong>Envio à CARRIER ainda não confirmado.</strong><p className="mt-2">Esta etapa não é manual. Ela será marcada automaticamente somente depois que o pedido for enviado por e-mail e o protocolo real de envio for gravado.</p><Link href={`/pedidos/${id}`} className="mt-4 inline-flex rounded-xl bg-cyan-500 px-4 py-3 font-semibold text-slate-950">Ir para envio do pedido</Link></div>:<>{proxima?.id==="FATURADO"&&<div className="mt-5 grid gap-4 md:grid-cols-2"><label className="block text-sm text-slate-300">Número da NF<input value={nf} onChange={e=>setNf(e.target.value)} className="mt-2 w-full rounded-xl border border-[#24466f] bg-[#020817] px-4 py-3" placeholder="Obrigatório"/></label><label className="block text-sm text-slate-300">Número de série da NF<input value={serieNf} onChange={e=>setSerieNf(e.target.value.toUpperCase())} className="mt-2 w-full rounded-xl border border-[#24466f] bg-[#020817] px-4 py-3" placeholder="Obrigatório para rastreio"/></label></div>}{proxima?.id==="INSTALADO"&&<label className="mt-5 block text-sm text-slate-300">Número de série instalado<input value={serieInstalada} onChange={e=>setSerieInstalada(e.target.value.toUpperCase())} className="mt-2 w-full rounded-xl border border-[#24466f] bg-[#020817] px-4 py-3" placeholder="Série efetivamente instalada"/><span className="mt-2 block text-xs text-slate-500">Pode ser diferente da série faturada. A divergência será preservada e sinalizada, não sobrescrita.</span></label>}<label className="mt-5 block text-sm text-slate-300">Observação<textarea value={observacao} onChange={e=>setObservacao(e.target.value)} rows={4} className="mt-2 w-full rounded-xl border border-[#24466f] bg-[#020817] px-4 py-3"/></label>{proxima?<button disabled={bloqueado} onClick={()=>void avancar(proxima.id)} className="mt-5 w-full rounded-xl bg-cyan-500 px-5 py-3 font-bold text-slate-950 disabled:opacity-50">{salvando?"Salvando...":`Confirmar: ${proxima.rotulo}`}</button>:<div className="mt-5 rounded-xl border border-emerald-800 bg-emerald-950/20 p-4 text-emerald-300">Ciclo operacional encerrado após instalação confirmada.</div>}</>}</article></section>
  </>}
 </div></section></main>
}
