...
function percentual(valor: unknown) { const numero = Number(valor || 0); return `${Math.round(numero <= 1 ? numero * 100 : numero)}%` }
function dataHora(valor?: string) { if (!valor) return "Data não informada"; const data = new Date(valor); return Number.isNaN(data.getTime()) ? valor : data.toLocaleString("pt-BR") }
...
export default function OportunidadeDetalhesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/crm-visao/oportunidades/${id}/detalhes`, { cache: "no-store" })
      .then(async (response) => { const payload = await response.json().catch(() => null); if (!response.ok) throw new Error(payload?.detail || "Não foi possível carregar os detalhes."); return payload as Detalhes })
      ...
      <div className="space-y-6 p-4 sm:p-6 lg:p-8">
        ...
        {dados && <>
          ...
          <section className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
            <article className="rounded-3xl border border-[#13203f] bg-[#071427] p-6"><h2 className="text-xl font-bold">Descrição comercial</h2><p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-slate-300">{contexto?.descricao || dados.oportunidade.descricao || "Nenhuma descrição registrada."}</p></article>
            <article className="rounded-3xl border border-[#13203f] bg-[#071427] p-6"><h2 className="text-xl font-bold">Resumo operacional</h2><dl className="mt-4 space-y-3 text-sm"><Linha label="Empresa" valor={String(dados.oportunidade.cliente_nome || "-")} /><Linha label="Produtos iniciais" valor={contexto?.equipamentos.join(", ") || "A definir"} /><Linha label="Quantidade inicial" valor={String(contexto?.quantidade || 1)} /><Linha label="Território" valor={[contexto?.municipio, contexto?.uf].filter(Boolean).join(" / ") || "Não informado"} /><Linha label="Fechamento previsto" valor={String(dados.oportunidade.data_fechamento_prevista || "Não informado")} /><Linha label="Movimentações" valor={String(dados.resumo.movimentacoes_pipeline)} /></dl></article>
          </section>

          <OportunidadeItensComerciais oportunidadeId={id} />

          <section className="rounded-3xl border border-[#13203f] bg-[#071427] p-6">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-xl font-bold">Linha do tempo comercial</h2><p className="mt-1 text-sm text-slate-400">Atividades, alterações de etapa, propostas, pedidos e registros históricos.</p></div><span className="text-sm text-cyan-300">{dados.eventos.length} evento(s)</span></div>
            <div className="mt-6 space-y-3">{dados.eventos.length === 0 ? <p className="text-sm text-slate-500">Nenhum evento vinculado.</p> : dados.eventos.map((evento, indice) => <article key={`${evento.tipo}-${evento.data_hora}-${indice}`} className="rounded-2xl border border-[#16325c] bg-[#091a33] p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><span className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-400">{evento.tipo}</span><h3 className="mt-1 font-semibold text-white">{evento.titulo || evento.tipo}</h3></div><div className="text-left text-xs text-slate-500 sm:text-right"><p>{dataHora(evento.data_hora)}</p>{evento.status && <p className="mt-1 text-cyan-300">{evento.status}</p>}</div></div></article>)}</div>
          </section>
        </>}
      </div>
    </section>
