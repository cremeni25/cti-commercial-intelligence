...
import { CheckCircle2, ExternalLink, FilePlus2, History, Loader2, Paperclip, RefreshCw } from "lucide-react"
...
type Dossie = { cadastro: Cadastro | null; ciclos?: Cadastro[]; documentos: Documento[] }
...
export default function DossieFinanceiroCarrier({ propostaId, compacto = false }: { propostaId: string; compacto?: boolean }) {
  ...
  const cicloAtual = Number(dossie?.cadastro?.ciclo_numero || 1)
  const vinculados = useMemo(() => dossie?.documentos.filter((d) => d.vinculado_proposta).length || 0, [dossie])
  const historico = useMemo(() => (dossie?.ciclos || []).filter((c) => !c.ciclo_atual), [dossie])
  ...
  async function anexar() {
    ...
    try {
      ...
      setMensagem(`Documento anexado ao cliente, ao ciclo ${cicloAtual} e vinculado a esta proposta.`)
      setObservacao("")
  ...
  async function alternarVinculo(doc: Documento) {
    ...
    try {
      ...
      setMensagem(doc.vinculado_proposta ? "Documento desvinculado desta proposta; permanece no histórico do cliente." : "Documento do cliente vinculado a esta proposta.")
      await carregar()
  ...
  async function salvarCadastro() {
    ...
    try {
      ...
      setMensagem(status === "VALIDADO_CARRIER"
        ? "Validação Carrier registrada. Validade calculada por 12 meses."
        : status === "RENOVACAO_EM_ANALISE"
          ? "Novo ciclo de renovação aberto. O ciclo anterior foi preservado integralmente no histórico."
  ...
  return <section className={`rounded-3xl border border-[#16325c] bg-[#07162b] ${compacto ? "p-5" : "p-6"}`}>
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-400">Crédito Carrier</p><h3 className={compacto ? "mt-1 font-bold" : "mt-1 text-xl font-bold"}>Dossiê financeiro do cliente</h3><p className="mt-1 text-sm text-slate-400">Cada validação forma um ciclo de 12 meses. Renovações preservam integralmente os ciclos e documentos anteriores.</p></div>
    ...
    {carregando ? <div className="grid min-h-24 place-items-center"><Loader2 className="animate-spin text-cyan-300"/></div> : <>
      <div className="mt-5 grid gap-3 sm:grid-cols-4">
        <div className="rounded-2xl border border-[#16325c] bg-[#091a33] p-4"><p className="text-xs text-slate-400">Ciclo atual</p><strong className="mt-1 block">Ciclo {cicloAtual}</strong></div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
        <label className="block"><span className="mb-2 block text-xs text-slate-400">Situação</span><select value={status} onChange={(e)=>setStatus(e.target.value)} className="w-full rounded-xl border border-[#24466f] bg-[#020817] px-3 py-3 text-sm"><option value="EM_PREPARACAO">Em preparação</option><option value="EM_ANALISE">Em análise pela Carrier</option><option value="VALIDADO_CARRIER">Validado pela Carrier</option><option value="RENOVACAO_EM_ANALISE">Iniciar renovação — novo ciclo</option></select></label>
      ...
      <div className="mt-6 rounded-2xl border border-[#16325c] p-4"><h4 className="font-semibold">Adicionar documento ao ciclo {cicloAtual}</h4><div className="mt-3 grid gap-3 sm:grid-cols-2"><select value={categoria} onChange={(e)=>setCategoria(e.target.value)} className="rounded-xl border border-[#24466f] bg-[#020817] px-3 py-3 text-sm">{categorias.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select><input ref={fileRef} type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png" className="rounded-xl border border-[#24466f] bg-[#020817] px-3 py-3 text-sm"/></div><input value={observacao} onChange={(e)=>setObservacao(e.target.value)} placeholder="Observação opcional" className="mt-3 w-full rounded-xl border border-[#24466f] bg-[#020817] px-3 py-3 text-sm"/><button type="button" onClick={()=>void anexar()} disabled={processando} className="mt-3 rounded-xl bg-cyan-500 px-4 py-3 text-sm font-bold text-slate-950 disabled:opacity-40"><FilePlus2 className="mr-2 inline" size={16}/>Anexar ao cliente e à proposta</button></div>

      <div className="mt-6 space-y-2"><h4 className="font-semibold">Documentos do cliente</h4>{!dossie?.documentos?.length && <p className="text-sm text-slate-400">Nenhum documento financeiro cadastrado.</p>}{dossie?.documentos?.map((doc)=><div key={doc.id} className="flex flex-col gap-3 rounded-2xl border border-[#16325c] bg-[#091a33] p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="truncate font-medium"><Paperclip className="mr-2 inline text-cyan-300" size={15}/>{doc.nome_arquivo}</p><p className="mt-1 text-xs text-slate-400">{categorias.find(([v])=>v===doc.categoria)?.[1] || doc.categoria}{doc.ciclo_numero ? ` · ciclo ${doc.ciclo_numero}` : ""}{doc.vinculado_proposta ? " · vinculado a esta proposta" : " · disponível no dossiê do cliente"}</p></div><div className="flex gap-2"><button type="button" onClick={()=>void abrir(doc)} className="rounded-lg border border-[#24466f] px-3 py-2 text-xs text-cyan-200"><ExternalLink className="mr-1 inline" size={14}/>Abrir</button><button type="button" disabled={processando} onClick={()=>void alternarVinculo(doc)} className="rounded-lg border border-[#24466f] px-3 py-2 text-xs disabled:opacity-40">{doc.vinculado_proposta ? "Desvincular da proposta" : "Vincular à proposta"}</button></div></div>)}</div>

      <div className="mt-6 rounded-2xl border border-[#16325c] bg-[#061224] p-4"><div className="flex items-center gap-2"><History size={17} className="text-cyan-300"/><h4 className="font-semibold">Histórico de ciclos Carrier</h4></div>{!historico.length ? <p className="mt-3 text-sm text-slate-400">Nenhum ciclo anterior. O histórico será preservado automaticamente quando houver renovação.</p> : <div className="mt-3 space-y-2">{historico.map((c)=><div key={String(c.id || c.ciclo_numero)} className="rounded-xl border border-[#16325c] bg-[#091a33] p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><strong>Ciclo {c.ciclo_numero || "—"}</strong><span className="text-cyan-200">{statusLabel[String(c.status_calculado || c.status || "")] || String(c.status_calculado || c.status || "—")}</span></div><p className="mt-1 text-xs text-slate-400">Validação: {dataBr(c.validado_carrier_em)} · validade: {dataBr(c.valido_ate)} · encerrado: {dataBr(c.encerrado_em)}</p></div>)}</div>}</div>
    </>}
  </section>
}
