...
const configs: Record<string, ModuloConfig> = {
  ...
  oportunidades: { titulo: "Oportunidades", endpoint: "/crm/oportunidades", icon: BriefcaseBusiness, extrair: (p) => (Array.isArray(p) ? p.filter((item) => String(item.origem || "").toUpperCase() === "CRM_APP") : []) },
  pipeline: { titulo: "Pipeline", endpoint: "/crm/oportunidades", icon: CircleDollarSign, extrair: (p) => (Array.isArray(p) ? p.filter((item) => String(item.origem || "").toUpperCase() === "CRM_APP") : []) },
...
const produtosPorLinha: Record<string, string[]> = {
  ...
  "DIESEL TRUCK": ["Supra 1150", "Supra 850", "Supra 750", "D7", "D7 AE", "A500"],
  "DIRECT DRIVE": ["CM600", "CM500", "CM400", "CM280", "Xarios 350", "Xarios 600"],
}
...
function texto(valor: unknown): string {
  ...
  if (typeof valor === "object") {
    const objeto = valor as Registro
    return texto(objeto.nome || objeto.razao_social || objeto.nome_fantasia || objeto.empresa || objeto.codigo || objeto.id)
  }
  return ""
}

function clienteDe(item: Registro): ClienteOpcao | null {
  const id = texto(item.id || item.cliente_id || item.uuid || item.codigo)
  const nome = texto(item.razao_social || item.nome || item.nome_fantasia || item.empresa || item.cliente)
  const codigo = texto(item.codigo || item.codigo_cliente || item.id)
  return id && nome ? { id, nome, codigo } : null
}
...
export default function CrmModuloPage() {
  ...
  const api = useCallback(async (endpoint: string, init?: RequestInit) => fetch(`/api/crm-proxy${endpoint}`, { ...init, cache: "no-store" }), [])
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      setRegistros(config.extrair(payload))
      if (clientesResposta.ok) {
        const dados = await clientesResposta.json()
        setClientes((Array.isArray(dados) ? dados : []).map(clienteDe).filter(Boolean) as ClienteOpcao[])
      }
    } catch (falha) {
  ...
  const sugestoesClientes = useMemo(() => {
    ...
    return clientes.filter((cliente) => `${cliente.nome} ${cliente.codigo}`.toLowerCase().includes(termo)).slice(0, 8)
  }, [clienteBusca, clienteSelecionado, clientes])

  const equipamentosDisponiveis = useMemo(() => linhas.flatMap((linha) => produtosPorLinha[linha] || []), [linhas])
  ...
  async function salvar(evento: FormEvent<HTMLFormElement>) {
    ...
    if (!clienteSelecionado || !userId) {
      setErro("Selecione um cliente da lista e confirme o usuário autenticado.")
      setSalvando(false)
      return
    }
    ...
    if (!payload.titulo) {
      setErro("Informe o título ou objetivo do registro.")
      setSalvando(false)
      return
    }
  ...
  return <main className="min-h-[100dvh] bg-[#020817] pb-24 text-white">
    ...
    <div className="mx-auto w-full max-w-[94vw] px-4 py-4 sm:px-6 sm:py-6">
      ...
      {novo ? <form onSubmit={salvar} className="mt-4 grid gap-4 rounded-3xl border border-[#16325c] bg-[#07162b] p-4 sm:grid-cols-2 sm:p-6">
        <label className="relative"><span className="mb-2 block text-sm text-slate-300">Cliente</span><input value={clienteBusca} onChange={(e) => { setClienteBusca(e.target.value); setClienteSelecionado(null) }} placeholder="Digite o nome do cliente" className="h-12 w-full rounded-2xl border border-[#24466f] bg-[#020817] px-4 outline-none focus:border-cyan-500" />{sugestoesClientes.length > 0 && <div className="absolute z-20 mt-2 max-h-64 w-full overflow-auto rounded-2xl border border-[#24466f] bg-[#07162b] shadow-2xl">{sugestoesClientes.map((cliente) => <button type="button" key={cliente.id} onClick={() => { setClienteSelecionado(cliente); setClienteBusca(cliente.nome) }} className="block w-full border-b border-[#16325c] px-4 py-3 text-left last:border-0"><strong className="block">{cliente.nome}</strong><span className="text-xs text-slate-400">Código incorporado: {cliente.codigo}</span></button>)}</div>}{clienteSelecionado && <span className="mt-2 block text-xs text-emerald-300">Cliente selecionado • código {clienteSelecionado.codigo}</span>}</label>
        <Campo name="titulo" label={slug === "visitas" ? "Objetivo da visita" : "Título"} required />
        {slug === "oportunidades" ? <>
          <Campo name="valor_estimado" label="Valor estimado" type="number" /><Campo name="probabilidade" label="Probabilidade (%)" type="number" />
          <SelecaoMultipla titulo="Linhas de produto" opcoes={Object.keys(produtosPorLinha)} selecionados={linhas} alterar={(valor) => { setLinhas(valor); setEquipamentos((atuais) => atuais.filter((item) => valor.some((linha) => produtosPorLinha[linha]?.includes(item)))) }} />
          <SelecaoMultipla titulo="Equipamentos" opcoes={equipamentosDisponiveis} selecionados={equipamentos} alterar={setEquipamentos} />
