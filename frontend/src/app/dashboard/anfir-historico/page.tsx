...
export default function AnfirHistoricoPage(){
 ...
 return <main className="flex min-h-screen bg-[#020817] text-white"><Sidebar/><section className="min-w-0 flex-1 overflow-hidden"><Topbar/><div className="space-y-6 p-4 sm:p-6 lg:p-8">
  ...
  {!loading&&!erro&&<><section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric titulo="Registros históricos filtrados" valor={fmt(historico?.metadata?.total_registros_filtrados)}/><Metric titulo="Clientes históricos" valor={fmt(historico?.total_clientes)}/><Metric titulo="Estados atendidos" valor={fmt(historico?.total_estados)}/><Metric titulo="Municípios" valor={fmt(historico?.total_municipios)}/></section><section className="rounded-2xl border border-[#17304d] bg-[#071427] p-5"><div className="mb-4"><h2 className="text-lg font-bold">Volume ANFIR por linha</h2><p className="mt-1 text-xs text-slate-500">{linhas?.metadata?.descricao||"Período selecionado comparado ao período anterior."}</p></div><div className="h-[340px]"><ResponsiveContainer width="100%" height="100%"><BarChart data={dados} margin={{top:30,right:18,left:0,bottom:0}}><CartesianGrid stroke="#17304d" strokeDasharray="3 5" vertical={false}/><XAxis dataKey="linha" stroke="#8294ad"/><YAxis stroke="#8294ad"/><Tooltip contentStyle={{backgroundColor:"#061126",border:"1px solid #24507a",borderRadius:"12px",color:"#fff"}}/><Legend/><Bar dataKey="anterior" name="Período anterior" fill="#52657d" radius={[7,7,0,0]}><LabelList dataKey="anterior" position="top" fill="#94a3b8" fontSize={12}/></Bar><Bar dataKey="atual" name="Período selecionado" fill="#22d3ee" radius={[7,7,0,0]}><LabelList dataKey="atual" position="top" fill="#67e8f9" fontSize={12}/></Bar></BarChart></ResponsiveContainer></div></section></>}
 </div></section></main>
}
