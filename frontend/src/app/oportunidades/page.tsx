...
export default function OportunidadesPage() {
  ...
  const [edicao, setEdicao] = useState({ valor: 0, chance: 0, quantidade: 1, status: "OPORTUNIDADE", data: "" })
  ...
  async function carregar() {
    ...
    try {
      const [oportunidadesResponse, clientesResponse] = await Promise.all([
        ...
        fetch(`${API_URL}/modulos/clientes?contexto=brasil&periodo=TODO_HISTORICO`, { cache: "no-store" }),
      ])
  ...
  async function criarOportunidade(event: FormEvent<HTMLFormElement>) {
    ...
    const linhas = String(form.get("linha_equipamentos") || "").split(",").map((item) => item.trim()).filter(Boolean)
    const equipamentos = String(form.get("equipamento") || "").split(",").map((item) => item.trim()).filter(Boolean)
    const quantidade = Math.max(1, Number(form.get("quantidade") || 1))
    const descricao = montarDescricaoComContexto(String(form.get("descricao") || ""), {
      linhas,
      equipamentos,
      quantidade,
    ...
    try {
      const response = await fetch(`${API_URL}/crm/oportunidades`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) })
  ...
  const abertas = useMemo(() => dados.filter((item) => !["GANHO", "PERDIDO", "CANCELADO"].includes(String(item.status || "").toUpperCase())), [dados])
  const valorPipeline = abertas.reduce((total, item) => total + Number(item.valor_estimado || 0), 0)
  const pipelinePonderado = abertas.reduce((total, item) => total + Number(item.valor_estimado || 0) * fator(item.probabilidade), 0)

  return <main className="flex min-h-screen bg-[#020817]">
    <Sidebar />
    <section className="min-w-0 flex-1">
      <Topbar />
      <div className="space-y-6 p-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><h1 className="text-4xl font-bold text-white">CRM • Oportunidades</h1><p className="mt-2 text-gray-400">Mesmos registros operacionais do App CRM, com edição e análise consolidada.</p></div><button type="button" onClick={() => setMostrarFormulario(true)} className="rounded-xl bg-cyan-500 px-5 py-3 font-semibold text-slate-950">Nova oportunidade</button></div>
        {erro && <div className="rounded-xl border border-red-500 p-4 text-red-300">{erro}</div>}

        {mostrarFormulario && <form onSubmit={criarOportunidade} className="rounded-2xl border border-cyan-700 bg-[#071226] p-6 text-gray-200">
          ...
          <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <Campo nome="probabilidade" label="Chance estimada de fechamento (%)" tipo="number" />
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-4"><Kpi titulo="Clientes disponíveis" valor={clientes.length.toLocaleString("pt-BR")} /><Kpi titulo="Oportunidades abertas" valor={abertas.length.toLocaleString("pt-BR")} /><Kpi titulo="Pipeline total" valor={valorPipeline.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })} /><Kpi titulo="Pipeline ponderado" valor={pipelinePonderado.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })} /></section>

        <div className="overflow-x-auto rounded-2xl border border-[#13203f] bg-[#091a33]">
          {loading ? <div className="p-10 text-gray-400">Carregando...</div> : dados.length === 0 ? <div className="p-10 text-gray-300">Nenhuma oportunidade cadastrada.</div> : <table className="w-full text-left"><thead><tr className="border-b border-[#13203f] text-gray-400"><th className="p-4">Cliente</th><th className="p-4">Oportunidade</th><th className="p-4">Produto / quantidade</th><th className="p-4">Etapa</th><th className="p-4">Valor</th><th className="p-4">Chance</th><th className="p-4">Ação</th></tr></thead><tbody>{dados.map((item) => { const contexto = lerContextoOportunidade(item); const emEdicao = editando === item.id; return <tr key={item.id} className="border-b border-[#13203f] align-top text-gray-200"><td className="p-4 font-semibold text-cyan-300">{item.cliente_id || "-"}</td><td className="p-4"><strong>{item.titulo}</strong>{contexto.descricao && <p className="mt-1 max-w-sm text-xs text-gray-400">{contexto.descricao}</p>}</td><td className="p-4">{contexto.equipamentos.join(", ") || "A definir"}<div className="text-xs text-gray-400">{emEdicao ? <input type="number" min="1" value={edicao.quantidade} onChange={(e) => setEdicao({ ...edicao, quantidade: Math.max(1, Number(e.target.value)) })} className="mt-2 w-24 rounded border border-[#24466f] bg-[#020817] p-2" /> : `${contexto.quantidade} unidade(s)`}</div></td><td className="p-4">{emEdicao ? <select value={edicao.status} onChange={(e) => setEdicao({ ...edicao, status: e.target.value })} className="rounded border border-[#24466f] bg-[#020817] p-2"><option>OPORTUNIDADE</option><option>PROPOSTA</option><option>NEGOCIACAO</option><option>GANHO</option><option>PERDIDO</option><option>CANCELADO</option></select> : item.status}</td><td className="p-4 text-green-400">{emEdicao ? <input type="number" min="0" step="0.01" value={edicao.valor} onChange={(e) => setEdicao({ ...edicao, valor: Number(e.target.value) })} className="w-36 rounded border border-[#24466f] bg-[#020817] p-2 text-white" /> : Number(item.valor_estimado || 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</td><td className="p-4">{emEdicao ? <input type="number" min="0" max="100" value={edicao.chance} onChange={(e) => setEdicao({ ...edicao, chance: Number(e.target.value) })} className="w-20 rounded border border-[#24466f] bg-[#020817] p-2" /> : `${percentual(item.probabilidade)}%`}</td><td className="p-4">{emEdicao ? <div className="flex gap-2"><button disabled={salvando} onClick={() => void salvarEdicao(item)} className="rounded bg-cyan-500 px-3 py-2 font-semibold text-slate-950">Salvar</button><button onClick={() => setEditando(null)} className="rounded border border-cyan-700 px-3 py-2 text-cyan-300">Cancelar</button></div> : <button onClick={() => iniciarEdicao(item)} className="rounded border border-cyan-700 px-3 py-2 text-cyan-300">Alterar</button>}</td></tr> })}</tbody></table>}
        </div>
      </div>
    </section>
  </main>
