  ...
  return <main className="flex min-h-screen bg-[#020817] text-white">
    ...
    <section className="min-w-0 flex-1"><Topbar /><div className="space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      {cadastroMestre && <section className="grid gap-5 xl:grid-cols-2">
        <PainelTempo titulo="REALIZADO" subtitulo="O que estas empresas já representaram." destaque="Histórico confirmado"><div className="grid gap-3 sm:grid-cols-2"><Kpi titulo="Empresas históricas" valor={dados.length.toLocaleString("pt-BR")} /><Kpi titulo="Registros" valor={dados.reduce((s, i) => s + i.quantidade_registros, 0).toLocaleString("pt-BR")} /><Kpi titulo="Chassis identificados" valor={totalChassis.toLocaleString("pt-BR")} /><Kpi titulo="Valor registrado na base" valor={moeda(valorHistorico)} /></div></PainelTempo>
        <PainelTempo titulo="EM CURSO" subtitulo="O que está sendo construído agora." destaque="CRM atual"><div className="grid gap-3 sm:grid-cols-2"><Kpi titulo="Empresas com negócio em curso" valor={new Set(negociosEmCurso.map((item) => item.cliente_id).filter(Boolean)).size.toLocaleString("pt-BR")} /><Kpi titulo="Negócios em curso" valor={negociosEmCurso.length.toLocaleString("pt-BR")} /><Kpi titulo="Pipeline atual" valor={moeda(pipelineAtual)} /><Kpi titulo="Propostas vigentes / Pedidos em curso" valor={`${propostasVigentes.length} / ${pedidosEmCurso.length}`} /></div></PainelTempo>
      </section>}
      <section className="rounded-2xl border border-[#13203f] bg-[#091a33] p-5 sm:p-6">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-xl font-bold">{cadastroMestre ? "Empresas — histórico e situação atual" : "Dados operacionais"}</h2><p className="mt-1 text-sm text-slate-400">Abra uma empresa para confrontar REALIZADO × EM CURSO.</p></div><div className="flex flex-col gap-2 sm:flex-row">{cadastroMestre && <select value={categoria} onChange={(e) => setCategoria(e.target.value)} className="rounded-xl border border-[#13203f] bg-[#071028] px-4 py-3 text-white"><option value="TODAS">Todas as categorias</option>{categorias.map((item) => <option key={item} value={item}>{item}</option>)}</select>}<input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder={cadastroMestre ? "Buscar empresa, chassi, placa ou implementadora" : "Buscar empresa"} className="rounded-xl border border-[#13203f] bg-[#071028] px-4 py-3 text-white" /></div></div>
        {loading ? <p className="mt-8 text-slate-400">Carregando dados reais...</p> : lista.length === 0 ? <p className="mt-8 text-slate-400">Nenhuma empresa encontrada para os filtros selecionados.</p> : <div className="mt-6 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b border-[#13203f] text-slate-400"><th className="p-3">Empresa</th><th className="p-3">REALIZADO</th>{cadastroMestre && <th className="p-3">EM CURSO</th>}<th className="p-3">Linhas</th>{cadastroMestre && <th className="p-3">Ação</th>}</tr></thead><tbody>{lista.map((item) => { const crm = crmPorNome.get(normalizar(item.nome)); const categoriaEmpresa = categoriaPorNome.get(normalizar(item.nome)) || "Categoria não classificada"; const opsEmCurso = crm?.oportunidades.filter(negocioEmCurso) ?? []; return <tr key={item.nome} className="border-b border-[#13203f] text-slate-200 hover:bg-cyan-500/5"><td className="p-3"><p className="font-semibold text-white">{item.nome}</p><p className="text-xs text-slate-500">{categoriaEmpresa}</p></td><td className="p-3"><p>{item.quantidade_registros} registros</p><p className="text-xs text-slate-500">{item.quantidade_chassis ?? 0} chassis • {moeda(item.valor_total)} na base</p></td>{cadastroMestre && <td className="p-3"><p>{opsEmCurso.length} negócio(s)</p><p className="text-xs text-emerald-300">{moeda(opsEmCurso.reduce((s, op) => s + Number(op.valor_estimado || 0), 0))} em pipeline</p></td>}<td className="p-3">{item.linhas?.join(", ") || "-"}</td>{cadastroMestre && <td className="p-3"><button type="button" onClick={() => setDetalheNome(item.nome)} className="rounded-lg border border-cyan-500 px-3 py-2 text-cyan-300 hover:bg-cyan-500/10">Abrir visão temporal</button></td>}</tr> })}</tbody></table></div>}
      </section>
    </div></section>
