...
from services.anfir_market_intelligence import (
...
def _mes(registro: dict) -> str | None:
...
def _q(registros: list[dict], ano: int, trimestre: int) -> int:
    return sum(
        ...
        if (data := data_registro(registro))
        and data.year == ano
        and ((data.month - 1) // 3 + 1) == trimestre
    )
...
def _status_por_segmento(registros: list[dict]):
    ...
    for codigo in SEGMENTOS:
        ...
        total = len(itens)
        categorias = Counter(categoria_competitiva(r) for r in itens)
        nao_participou = sum(1 for r in itens if causa_estrategica(r) == "COBERTURA_COMERCIAL")
...
def _mensal(registros: list[dict]):
    ...
    for mes_num in range(1, 13):
        ...
        linhas.append({
            ...
            "trailer": contagem.get("TR", 0),
            "diesel_truck": contagem.get("DT", 0),
            "direct_drive": contagem.get("DD", 0),
            "total": len(itens),
        })
    return linhas
...
def _territorio(registros: list[dict]):
    ...
    for ddd in DDD_VIENA:
        itens = [r for r in registros if resolver_ddd_registro(r) == ddd]
        contagem = Counter(_linha(r) for r in itens)
        linhas.append({
            "ddd": ddd,
            "mercado": len(itens),
            ...
            "diesel_truck": contagem.get("DT", 0),
            "direct_drive": contagem.get("DD", 0),
            "responsavel": RESPONSAVEIS_DDD[ddd],
        })
    return linhas


def _leitura_cliente(dados: dict) -> str:
    if dados["nao_participou"] >= 3:
        return "Recuperar cobertura: cliente aparece repetidamente sem participação Carrier."
    if dados["sem_contato"] >= 2:
        return "Ativar contato e qualificar decisor/necessidade antes da próxima compra."
    if dados["preco"] >= 2:
        return "Trabalhar valor total, comparação técnica e condição comercial."
    if dados["relacionamento"] >= 2:
        return "Plano de relacionamento e cadência comercial."
    if dados["gap_tecnico"] >= 2:
        return "Validar aderência de produto/solução técnica."
    if dados["tk"] >= 3:
        return "Conta de conversão competitiva relevante."
    return "Qualificar oportunidade e histórico."


def _oportunidades_prioritarias(registros: list[dict], limite: int = 40):
    grupos = defaultdict(lambda: {
        "mercado": 0, "nao_carrier": 0, "carrier": 0, "nao_participou": 0,
        "sem_contato": 0, "tk": 0, "nacional": 0, "preco": 0,
        "relacionamento": 0, "gap_tecnico": 0,
    })
    for registro in registros:
        ...
        if causa == "TECNICO_PRODUTO": dados["gap_tecnico"] += 1

    retorno = []
    for cliente, dados in grupos.items():
        score = (
            dados["nao_carrier"]
            + 2 * (dados["nao_participou"] + dados["sem_contato"])
            + dados["preco"] + dados["relacionamento"] + dados["gap_tecnico"]
        )
        prioridade = "ALTA" if score >= 15 else "MÉDIA" if score >= 9 else "MONITORAR"
        retorno.append({
            "prioridade": prioridade,
            "cliente": cliente,
            "score_transparente": score,
            **dados,
            "leitura_sugerida": _leitura_cliente(dados),
            "criterio_score": "não Carrier + 2×(não participou + sem contato) + preço + relacionamento + gap técnico",
        })
...
def _implementadoras(registros: list[dict], limite: int = 30):
    ...
    for nome, contagem in grupos.items():
        ...
        if nome == "NÃO INFORMADA":
            continue
        dominante = max(SEGMENTOS, key=lambda codigo: contagem.get(codigo, 0))
        retorno.append({
            "implementadora": nome,
            "mercado_elegivel": total,
            "trailer": contagem.get("TR", 0),
            "diesel_truck": contagem.get("DT", 0),
            "direct_drive": contagem.get("DD", 0),
            "leitura": f"Canal relevante em {NOMES_SEGMENTOS[dominante]}; cruzar status competitivo e clientes para plano de relacionamento.",
        })
...
TEMA_CONTRATO = {
    "TECNICO_PRODUTO": ("Há casos em que a limitação percebida de solução/produto influencia a decisão; exige validação técnica antes de classificar como perda por preço.", "Alerta de gap técnico + fila para engenharia/produto"),
    "POS_VENDA_MANUTENCAO": ("Condições e promessas de manutenção/pós-venda aparecem como argumento competitivo e precisam ser comparadas de forma objetiva.", "Comparativo de oferta/pós-venda e objeções"),
    ...
    "USADO_REAPROVEITAMENTO": ("Uso/reaproveitamento de equipamento ou implemento altera a oportunidade: muitas ocorrências são renovação/legalização, não compra nova convencional.", "Radar de renovação/substituição de frota"),
    "IMPLEMENTADORA_INTEGRADA": ("Quando implementadora/fabricante também fornece a solução frigorífica, a decisão pode ocorrer antes do contato comercial da Viena.", "Mapa de influência das implementadoras"),
    ...
    "TESTE_DEMO": ("Há casos com participação, teste ou demo que não converteram; isso permite medir efetividade da ação comercial.", "Funil teste/demo → proposta → venda"),
...
def _observacoes(registros: list[dict]):
    ...
    uteis = 0
    for registro in registros:
        texto = _texto(registro.get("ocorrencia"))
        if texto:
            uteis += 1
        for tema in temas_observacao(registro):
            ...
            if texto:
                observacoes[tema].add(texto)

    temas = []
    for tema, quantidade in contagem.most_common():
        leitura, uso = TEMA_CONTRATO.get(tema, (
            "Sinal qualitativo extraído da observação original; requer leitura contextual.",
            "Contexto para análise humana/IA; não usar como classificação cega",
        ))
        temas.append({
            "tema": tema,
            "ocorrencias": quantidade,
            "clientes_unicos": len(clientes[tema]),
            "observacoes_unicas": len(observacoes[tema]),
            "leitura_comercial": leitura,
            "uso_no_cti": uso,
        })
    return {
        "temas": temas,
        "registros_elegiveis": len(registros),
        "com_observacao_util": uteis,
        "cobertura_observacoes_percentual": _percentual(uteis, len(registros)),
        "sem_observacao_util": len(registros) - uteis,
        "regra": "Texto original preservado; temas derivados são auxiliares e podem coexistir.",
    }
...
def consolidar_workbook_anfir_2026(registros: list[dict]):
    ...
    categorias = Counter(categoria_competitiva(r) for r in registros)
    nao_participou = sum(1 for r in registros if causa_estrategica(r) == "COBERTURA_COMERCIAL")
    ...
    return {
        "metadata": {
            "nome_contrato": "Auditoria_ANFIR_Carrier_JOV_2026_Inteligencia_Viena_Observacoes",
            "competencia": "2026",
            "natureza": "FOTOGRAFIA_ANFIR_CARRIER_JOV_VIENA",
            "market_share_oficial": False,
        },
        "inteligencia_viena": {
            "mercado_elegivel": len(registros),
            "carrier_observada": categorias.get("CARRIER", 0),
            "carrier_presenca_percentual": _percentual(categorias.get("CARRIER", 0), len(registros)),
            "sem_contato": categorias.get("SEM_CONTATO", 0),
            "nao_participamos_proposta": nao_participou,
            "dados_status_a_qualificar": categorias.get("NAO_CLASSIFICADO", 0),
            "q1": q1,
            "q2": q2,
            "q2_vs_q1_percentual": round((q2 - q1) / q1 * 100, 2) if q1 else None,
            "segmentos": segmentos,
            "mensal": _mensal(registros),
            "territorio": _territorio(registros),
            "leituras_estrategicas": leituras,
        },
