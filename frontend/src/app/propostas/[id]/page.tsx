...
export default function PropostaPage() {
  ...
  async function solicitarAceite(metodo: "PRESENCIAL_TELA" | "REMOTO_LINK") {
    ...
    const payload = await executar(`/crm-documentos/propostas/${id}/aceites`, {
      metodo,
      ...
      <div className="space-y-6 p-4 sm:p-6 lg:p-8">
        <header className="rounded-3xl border border-[#13203f] bg-[#091a33] p-6">
          ...
          <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-cyan-400">Proposta comercial</p><h1 className="mt-2 text-3xl font-bold">{texto(dados?.proposta.numero, "Proposta em elaboração")}</h1><p className="mt-2 text-slate-400">Versão {texto(dados?.proposta.versao, "1")} • {texto(dados?.item?.equipamento)}</p></div>
            <span className="w-fit rounded-full border border-cyan-800 bg-cyan-950/30 px-4 py-2 text-sm text-cyan-200">{status.replaceAll("_", " ")}</span>
          </div>
        </header>

        {carregando && <Aviso>Carregando proposta...</Aviso>}
        ...
        {dados && <>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <Kpi titulo="Valor" valor={moeda(dados.proposta.valor)} />
            <Kpi titulo="Quantidade" valor={texto(dados.item?.quantidade, "1")} />
            <Kpi titulo="Aceites" valor={String(dados.aceites.length)} />
            <Kpi titulo="Pedidos" valor={String(dados.pedidos.length)} />
          </section>

          <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            <article className="rounded-3xl border border-[#13203f] bg-[#071427] p-6">
              <h2 className="text-xl font-bold">Dados comerciais</h2>
              <dl className="mt-5 space-y-3 text-sm">
                <Linha label="Cliente" valor={texto(dados.cliente?.razao_social || dados.cliente?.nome || dados.oportunidade?.cliente_nome)} />
                <Linha label="Linha" valor={texto(dados.item?.linha_produto)} />
                <Linha label="Equipamento" valor={texto(dados.item?.equipamento)} />
                <Linha label="Configuração" valor={texto(dados.item?.configuracao)} />
                <Linha label="Preço unitário" valor={moeda(dados.item?.preco_unitario)} />
                <Linha label="Desconto" valor={`${texto(dados.item?.desconto_percentual, "0")}%`} />
                ...
                <Linha label="Local de entrega" valor={texto(dados.item?.local_entrega)} />
              </dl>
            </article>
            ...
            <article className="rounded-3xl border border-[#13203f] bg-[#071427] p-6">
              ...
              <div className="mt-5 grid gap-3">
                <button disabled={!podeEmitir || processando} onClick={() => void executar(`/crm-documentos/propostas/${id}/emitir`)} className="rounded-xl bg-cyan-500 px-4 py-3 font-semibold text-slate-950 disabled:opacity-40">Emitir proposta</button>
                <button disabled={!podeAceite || processando} onClick={() => void solicitarAceite("PRESENCIAL_TELA")} className="rounded-xl border border-cyan-700 px-4 py-3 text-cyan-200 disabled:opacity-40">Aceite presencial</button>
                <button disabled={!podeAceite || processando} onClick={() => void solicitarAceite("REMOTO_LINK")} className="rounded-xl border border-cyan-700 px-4 py-3 text-cyan-200 disabled:opacity-40">Gerar link de aceite</button>
                <button disabled={!podePedido || processando} onClick={() => void executar(`/crm-documentos/propostas/${id}/converter-pedido`, {})} className="rounded-xl border border-emerald-700 px-4 py-3 text-emerald-300 disabled:opacity-40">Gerar pedido</button>
                <button onClick={() => window.print()} className="rounded-xl border border-slate-700 px-4 py-3 text-slate-200">Imprimir / salvar PDF</button>
              </div>
            </article>
          </section>

          <section className="rounded-3xl border border-[#13203f] bg-[#071427] p-6">
            <h2 className="text-xl font-bold">Auditoria do documento</h2>
            <div className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
              <Linha label="Hash" valor={texto(dados.proposta.hash_documento)} />
              <Linha label="Modelo" valor={texto(dados.proposta.modelo_proposta_id, "Modelo provisório")}/>
              <Linha label="Emitida em" valor={texto(dados.proposta.emitida_em)} />
              <Linha label="Aceita em" valor={texto(dados.proposta.aceita_em)} />
            </div>
            <details className="mt-5 rounded-2xl border border-[#13203f] p-4"><summary className="cursor-pointer text-sm font-semibold text-cyan-300">Snapshot imutável</summary><pre className="mt-4 overflow-x-auto whitespace-pre-wrap text-xs text-slate-400">{JSON.stringify(snapshot, null, 2)}</pre></details>
          </section>
        </>}
      </div>
    </section>
