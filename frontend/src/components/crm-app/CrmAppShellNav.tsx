...
const textos = {
  ...
  es: {
    ...
    archiveError: "No fue posible archivar el registro de prueba.", failure: "Fallo",
  },
} as const
...
export function CrmAppShellNav() {
  ...
  async function arquivarTeste() {
    ...
    if (!window.confirm(tx.archiveConfirm)) return
    setArquivando(true)
    setErro("")
    try {
      ...
      if (!resposta.ok) throw new Error(String(payload?.detail || `${tx.failure} ${resposta.status}`))
      router.push("/crm-app/oportunidades")
      router.refresh()
    } catch (falha) {
      setErro(falha instanceof Error ? falha.message : tx.archiveError)
    } finally {
      setArquivando(false)
    }
  }
  ...
  return (
    <>
          ...
          <div className={`grid gap-2 ${adminMaster ? "grid-cols-2" : "grid-cols-1"}`}>
            ...
            {adminMaster && (
              <button type="button" disabled={arquivando} onClick={() => void arquivarTeste()} className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-amber-700 bg-amber-950/40 px-3 text-sm font-semibold text-amber-200 disabled:opacity-50">
                <Archive size={17} />
                {arquivando ? tx.archiving : tx.archiveTest}
              </button>
            )}
          </div>
        </div>
      ...
      <nav aria-label={tx.navLabel} className="fixed inset-x-0 bottom-0 z-50 border-t border-[#16325c] bg-[#061126]/98 px-3 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 text-white backdrop-blur">
