...
type Resumo = { visitas: number; pendencias: number; oportunidades: number; pipeline: number; clientes: number; atividades: number }

export default function CrmAppPage() {
  const { usuario } = useAuth()
  const [resumo, setResumo] = useState<Resumo>({ visitas: 0, pendencias: 0, oportunidades: 0, pipeline: 0, clientes: 0, atividades: 0 })
  ...
  const sincronizar = useCallback(async () => {
    ...
    try {
      ...
      const abertas = oportunidadesApp.filter((item) => !["GANHO", "PERDIDO", "CANCELADO"].includes(String(item.status || "").toUpperCase())).length
      setResumo({ visitas: visitasHoje, pendencias: Number(agenda.resumo?.atrasadas || 0) + Number(agenda.resumo?.hoje || 0), oportunidades: abertas, pipeline: oportunidadesApp.length, clientes: Array.isArray(clientes) ? clientes.length : 0, atividades: atividades.length })
      setOnline(true)
    } catch {
      setOnline(false)
    } finally {
  ...
  const analises = [
    { label: "Agenda", valor: resumo.pendencias, descricao: "ações pendentes", icon: CalendarDays },
    { label: "Carteira", valor: resumo.clientes, descricao: "clientes disponíveis", icon: Building2 },
    { label: "Visitas", valor: resumo.visitas, descricao: "realizadas hoje", icon: MapPinned },
    { label: "Oportunidades", valor: resumo.oportunidades, descricao: "negócios abertos", icon: CircleDollarSign },
    { label: "Pipeline", valor: resumo.pipeline, descricao: "oportunidades do app", icon: TrendingUp },
    { label: "Atividades", valor: resumo.atividades, descricao: "interações registradas", icon: Activity },
  ]
  ...
  return <main className="min-h-[100dvh] bg-[#020817] pb-24 text-white">
    ...
    <div className="mx-auto w-full max-w-[94vw] px-4 py-4 sm:px-6 sm:py-6">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(360px,0.65fr)]">
        ...
        <section><h2 className="mb-4 text-xl font-semibold">Análises do CRM</h2><div className="grid grid-cols-2 gap-4 xl:grid-cols-3">{analises.map(({ label, valor, descricao, icon: Icon }) => <article key={label} className="flex min-h-32 flex-col justify-between rounded-2xl border border-[#16325c] bg-[#07162b] p-5"><Icon className="text-cyan-300" size={24} /><div><strong className="mt-3 block text-2xl text-cyan-300">{valor}</strong><span className="block font-semibold">{label}</span><span className="text-xs text-slate-400">{descricao}</span></div></article>)}</div><div className="mt-4 rounded-2xl border border-emerald-900/70 bg-emerald-950/20 p-5"><p className="font-semibold text-emerald-200">Leitura operacional conectada</p><p className="mt-1 text-sm leading-6 text-emerald-100/70">Os indicadores não criam registros. Eles consolidam somente as interações realizadas pelos botões operacionais.</p></div></section>
      </div>
    </div>
